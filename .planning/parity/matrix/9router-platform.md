...
| PAR-PLAT-047 | g0router OAuth flow: PKCE authorization-code for anthropic only; start + callback + refresh endpoints | `internal/auth/oauth.go:33-160` | HAVE-advanced | w7-prov-oauth: 8 provider flows added (claude/codex/gemini-cli/iflow/cline redirect; qwen/github/kilocode device-code) via additive config factories + a new device-code path (`internal/auth/oauth_device.go`); refresh quirks (iflow Basic-auth, cline JSON, kilocode/github none) via additive `OAuthConfig` flags. cursor (import_token) / kiro (AWS-SSO) / gitlab (PKCE) / antigravity (Google) remain out of scope (brief's 8). |
