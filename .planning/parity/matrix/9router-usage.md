...
| PAR-USAGE-013 | `getUsageStats` supports periods: today, 24h, 7d, 30d, 60d, all | `src/app/api/usage/stats/route.js:4` | HAVE | GET /api/usage/stats periods today/24h/7d/30d/60d/all (w5-d) |
| PAR-USAGE-014 | Daily-summary path used for periods >24h; live-history path used for today/24h | `src/lib/db/repos/usageRepo.js:418` | HAVE | Daily-summary path >24h vs live request_log path for today/24h (w5-d) |
| PAR-USAGE-015 | Stats include totalRequests, totalPromptTokens, totalCompletionTokens, totalCost | `src/lib/db/repos/usageRepo.js:367-370` | HAVE | Totals + pending/active/recent/errorProvider in stats payload; live token/cost fixed in fix-r1 (w5-d) |
| PAR-USAGE-016 | Stats break down byProvider, byModel, byAccount, byApiKey, byEndpoint | `src/lib/db/repos/usageRepo.js:370` | HAVE | Five breakdowns byProvider/byModel/byAccount/byApiKey/byEndpoint with ref key shapes (w5-d) |
| PAR-USAGE-017 | last10Minutes bucket array computed from usageHistory with 1-minute buckets | `src/lib/db/repos/usageRepo.js:394-416` | HAVE | last10Minutes 1-minute buckets, injected clock (w5-d) |
...
| PAR-USAGE-021 | `getChartData` returns 24 hourly buckets for today/24h, daily buckets for 7d/30d/60d | `src/app/api/usage/chart/route.js:4` | HAVE | GET /api/usage/chart: 24 hourly buckets today/24h, N daily zero-filled (w5-d) |
| PAR-USAGE-022 | Chart bucket labels use locale time strings for hours, short date for days | `src/lib/db/repos/usageRepo.js:631,673` | HAVE | Chart labels 15:04 / Jan 2 (locale-string adaptation recorded) (w5-d) |
| PAR-USAGE-023 | `getRecentLogs` derives logs from usageHistory with formatted string output | `src/lib/db/repos/usageRepo.js:701-730` | HAVE | GET /api/usage/request-logs + /api/usage/logs derived line format (w5-d) |
...
| PAR-USAGE-029 | Pricing API GET returns merged user + default pricing | `src/app/api/pricing/route.js:9-20` | HAVE | GET /api/pricing merged user+default (w5-d) |
| PAR-USAGE-030 | Pricing API PATCH validates fields input/output/cached/reasoning/cache_creation as non-negative numbers | `src/app/api/pricing/route.js:27-83` | HAVE | PATCH /api/pricing field whitelist + non-negative validation; returns user pricing (w5-d) |
| PAR-USAGE-031 | Pricing API DELETE resets per-provider, per-model, or all pricing | `src/app/api/pricing/route.js:91-117` | HAVE | DELETE /api/pricing per-provider/per-model/all + kv Delete/Clear (w5-d) |
| PAR-USAGE-032 | Provider usage API fetches external quotas for GitHub, Gemini, Antigravity, Claude, Codex, Kiro, GLM, MiniMax | `src/app/api/usage/[connectionId]/route.js:16,122-188` | PARTIAL | w5-e: dispatcher + claude (OAuth-first→legacy) + gemini (quota+subscription). w7-usage-quota: +antigravity (BUILT — gemini Cloud-Code twin, hermetic test). github/codex/kiro/glm/minimax DEFERRED under ESC-REF-ABSENT (frozen route.js absent on this host → concrete usage endpoint/auth/shape unconfirmable; github also blocked by ESC-GH-COPILOT token-mint deferral, kiro by ESC-KIRO-SIGV4 risk): each ships a graceful provider-named fallback arm (no network, unit-tested via TestProviderUsageDispatch), NO endpoint fabricated. Built-vs-deferred: BUILT={antigravity,claude,gemini}; DEFERRED-with-tested-fallback={github,codex,kiro,glm,minimax}. Stays PARTIAL because 5/8 providers do not yet fetch real external quotas; flip to HAVE when route.js confirms their endpoints. See open-questions.md w7-usage-quota. |
...
| PAR-USAGE-036 | Dashboard UsageStats component fetches `/api/usage/stats?period=`, subscribes to `/api/usage/stream` | `ui/src/components/usage/usage-stats.tsx` | HAVE | w6-g §1.2/§1.3 — REST `/api/usage/stats?period=` + additive `EventSource("/api/usage/stream")` overlay |
| PAR-USAGE-037 | Dashboard RequestLogger polls `/api/usage/request-logs` every 3s with auto-refresh toggle | `ui/src/components/usage/request-logger.tsx` | HAVE | w6-g — `/api/usage/request-logs` + 3s setInterval poll + auto-refresh toggle |
...
| PAR-USAGE-039 | Usage daily aggregation overlays precise lastUsed timestamps from history rows | `src/lib/db/repos/usageRepo.js:506-530` | HAVE | lastUsed overlay of precise timestamps onto daily aggregates (w5-d) |
