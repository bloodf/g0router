...
| PAR-BF-GOV-006 | VK value encrypted at rest: SHA-256 hash for lookup, vault or AES encryption in `BeforeSave` | `framework/configstore/tables/virtualkey.go:260-284` | HAVE | bf-gov-5: VK value now hash-for-lookup + AES-`key_enc` at rest in `virtual_keys` table — `key` holds `sha256hex(raw)` for O(1) lookup (`GetVirtualKeyByKey` hashes input, `internal/store/virtualkeys.go`), `key_enc` holds the reversible AES-256-GCM ciphertext (`crypto.go` cipher); `scanVirtualKey` decrypts `key_enc` into `.Key` (always plaintext for DTO/spend/gate), legacy plaintext rows migrated by an idempotent in-`Open` backfill (no operator lockout). RESIDUAL: `request_log.api_key` retains the raw VK value for spend attribution (now correlated in Go via decrypt in `SumCostByTeam`, NOT via a broken hash-column subquery) — encrypting the VK value in `request_log` is a separate, larger hardening, not claimed here. GOV-006 scope = the VK record/table; the request_log audit row is out of scope. |
...
| PAR-BF-GOV-013 | Budget atomic CAS increment via `BumpBudgetUsage`: auto-resets if ResetDuration expired, retries on CAS failure | `plugins/governance/store.go:375-410` | VAR | matrix-audit (post-bf-gov-4): budget ENFORCEMENT with period-reset is achieved variant-by-design via SQL-LIVE aggregation over `request_log`, not in-memory CAS accumulation — `QuotaEngine.Evaluate` (`internal/governance/quota.go:207`) → `checkTeamBudget` (`:315`) → `SumCostByTeam` (`internal/store/requestlog.go`); period reset is inherent in `windowStart(vk.BudgetPeriod)` (`quota.go:237`, daily/weekly/monthly) — no accumulator to CAS-reset. Same VAR-by-design family as 016/023/044/045. |
...
| PAR-BF-GOV-015 | Budget calendar-aligned reset: `IsCalendarAligned` derived from owner; reset path reads stamped value | `framework/configstore/tables/budget.go:36` | HAVE | bf-gov-3: the rate-limit dimensions' calendar window REUSES bf-gov-1's shipped `windowStart(period)` (`internal/governance/quota.go` daily/weekly/monthly) + injectable clock; the owner-derived `IsCalendarAligned` propagation via GORM AfterFind is ESC, g0router stamps the period directly (D2). |
| PAR-BF-GOV-016 | Budget DB sync every 10s: `DumpBudgets` writes in-memory `CurrentUsage` to database | `plugins/governance/store.go:1968` | VAR/ESC | bf-gov-3: budgets are SQL-live (bf-gov-1 D8 `SumCostBy*` — nothing in memory to dump); a 10s DB-sync worker is N/A by-design, same divergence class as the ESC'd `sync.Map` store (D4, open-questions). |
| PAR-BF-GOV-017 | Rate limit schema: dual token/request limits with MaxLimit, ResetDuration, CurrentUsage, LastReset per dimension | `framework/configstore/tables/ratelimit.go:11-42` | PARTIAL | bf-gov-3: dual token+request limits HAVE, BOTH SQL-live — `schemas.RateLimit{TokenMax,TokenResetPeriod,RequestMax,RequestResetPeriod}` (`internal/schemas/governance.go`) enforced in `Evaluate` via live `SumTokensByAPIKey`/`SumRequestsByAPIKey` aggregates (D1/D3). The persisted-counter `CurrentUsage`/`LastReset` schema is VAR-by-design (SQL-derived, not stored); CAS bump ESC. |
| PAR-BF-GOV-018 | Rate limit atomic CAS increment via `BumpRateLimitUsage`: auto-resets expired counters, retries on CAS failure | `plugins/governance/store.go:412-456` | PARTIAL | bf-gov-3: SQL-accrual variant HAVE — g0router "accrues" via the persisted `request_log` row (`SaveUsage`) and checks the LIVE aggregate (`checkTokenLimit`/`checkRequestLimit`); lost-increment safety is the DB write, not CompareAndSwap. CAS-spin mechanism ESC (D1, open-questions). |
| PAR-BF-GOV-019 | Rate limit calendar-aligned reset support via `GetCalendarPeriodStart` | `plugins/governance/store.go:1750` (referenced by reset worker) | HAVE | bf-gov-3: the rate-limit lazy reset uses the SAME shipped `windowStart(period)` the budget uses; the windowStart default branch was extended additively to parse rolling tokens (1h/1d/1M), 3 calendar cases UNCHANGED (D2). |
...
| PAR-BF-GOV-021 | `UsageUpdate` struct with streaming-aware fields: IsStreaming, IsFinalChunk, HasUsageData | `plugins/governance/tracker.go:17-31` | VAR | bf-gov-3: g0router's `SaveUsage` (`internal/store/requestlog.go`) IS the streaming-aware finalized write — one `request_log` row per COMPLETED request with final usage (verified `internal/api/usage_glue_test.go:721`); the SQL-live aggregates honor it inherently, so no `UsageUpdate` type is built (D4, open-questions). |
| PAR-BF-GOV-022 | `UpdateUsage` order: global provider/model → user-level → per-user scoped model → VK-level → per-VK scoped model | `plugins/governance/tracker.go:70-173` | PARTIAL | bf-gov-3: the VK→Team slice IS the persisted `request_log` row consumed live by the dual-dimension aggregates (D1/D3); global/user/per-user-scoped tiers ESC (same slice boundary as bf-gov-1). No `Accrue` method (D4). |
| PAR-BF-GOV-023 | Background reset worker: every 10s (`workerInterval`) resets expired rate limits and budgets, dumps to DB | `plugins/governance/tracker.go:49,199` | VAR/ESC | bf-gov-3: g0router governance is fully SQL-live (no in-memory accumulator); a 10s in-memory→DB sync worker is N/A by-design — restart-survival is automatic (every dimension reads persisted `request_log` live), calendar reset is inherent in the windowStart-bounded aggregate. Same divergence class as the ESC'd `sync.Map` store (D4, open-questions). |
...
| PAR-BF-GOV-026 | `WhiteList` semantics: `["*"]` allows all values; empty list denies all; non-empty list without `*` allows only listed values | `core/schemas/account.go:22-30` | HAVE | bf-gov-2: `schemas.WhiteList.IsAllowed` (`internal/schemas/lists.go:18`) encodes the matrix contract incl. empty=deny-all (unit-tested, `lists_test.go`). The GATE (`matchProviderConfig`, `vk.go:123`) deliberately PRESERVES legacy match-all on empty `AllowedModels` (nil OR empty) for backward-compat — VAR (D1), so no existing VK is silently 403'd; the gate does NOT adopt empty=deny-all (open-questions). |
| PAR-BF-GOV-027 | `BlackList` semantics: `["*"]` blocks all values; empty list blocks none; non-empty list without `*` blocks only listed values | `core/schemas/account.go:80-106` | HAVE | bf-gov-2: `schemas.BlackList.IsBlocked` (`internal/schemas/lists.go:51`) — empty blocks none, `["*"]` blocks all, else membership (D2). |
| PAR-BF-GOV-028 | Blacklist wins over allowlist: two-pass check (blacklist scan first, then allowlist scan) in `isModelAllowed` | `plugins/governance/resolver.go:358-390` | HAVE | bf-gov-2: `matchProviderConfig` (`internal/api/vk.go:116-124`) runs the blacklist pass first (blocked model skips the config) then the allowlist pass; a model in both lists is denied (blacklist wins, D3). VK provider-config slice. |
| PAR-BF-GOV-029 | Provider key-level blacklists: `TableKey.BlacklistedModels` runtime field persisted as `BlacklistedModelsJSON` | `framework/configstore/tables/key.go:24,83` | MISSING (ESC) | bf-gov-2 ESC: g0router has NO provider-key model store (`schemas.Key` is a bare credential, `store.Connection` persists secrets only, keys pinned via `KeyIDs`). A key-level blacklist would be a dead field with no live single-key model-gate (no-leftovers §3). ESC until a provider-key model-policy store is funded (open-questions). |
| PAR-BF-GOV-030 | VK provider config-level blacklists: `TableVirtualKeyProviderConfig.BlacklistedModels` persisted as JSON | `framework/configstore/tables/virtualkey.go:32` | HAVE | bf-gov-2: `schemas.ProviderConfig.BlacklistedModels schemas.BlackList` (`governance.go:17`), additive — rides the existing `config_json` blob (NO new column), consumed by the D3 blacklist pass. |
...
| PAR-BF-GOV-034 | Virtual key mandatory mode: rejects requests without `x-bf-vk` header when `isVkMandatory` is true | `plugins/governance/main.go:783-797` | HAVE (variant header) | bf-gov-4: `vk_mandatory` feature flag (seeded OFF, INSERT OR IGNORE — backward-compat by construction) gates an injected `mandatory func() bool` predicate on `VKGate` (`internal/api/vk.go`). `AllowVK("")` returns `(false, 401, "virtual key required", nil)` when mandatory ON. The 10 `/v1/*` call sites call `AllowVK` unconditionally (guard removed, Option-A). The `/mcp` `admitMCPVK` (`internal/admin/mcp.go`) mirrors via `h.vkMandatory()` — absent VK ON → JSON-RPC error "virtual key required" (Post) / 401 (SSE). Flag OFF (default/fresh/upgraded) → absent VK allowed everywhere, byte-identical to pre-bf-gov-4. VAR: header name `x-bf-vk → x-g0-vk`; key prefix `vk_ → g0vk-` (BIFROST-MAP §250 authorizes recording as variant). Ref `plugins/governance/main.go:783-797` ABSENT (ESC-REF-ABSENT); built to matrix behavior + g0router conventions only. |
| PAR-BF-GOV-035 | `Decision` enum for governance outcomes: Allow, VirtualKeyNotFound, VirtualKeyBlocked, RateLimited, BudgetExceeded, TokenLimited, RequestLimited, ModelBlocked, ProviderBlocked, MCPToolBlocked | `plugins/governance/resolver.go:14-27` | HAVE | bf-gov-3: `governance.Decision` typed enum (`internal/governance/quota.go`) with the gov-3-emitted subset (Allow/Budget/Rate/Token/RequestLimited); Model/Provider/MCP values are DECLARED as the shared enum bf-gov-2 (lists)/bf-mcp tiers emit (D8). Surfaced live as `error.code` via `api.DecisionCodeForReason` (`internal/api/vk.go`). |
| PAR-BF-GOV-036 | `EvaluationResult` carries Decision, Reason, VirtualKey, RateLimitInfo, BudgetInfo, UsageInfo | `plugins/governance/resolver.go:37-62` | PARTIAL | bf-gov-3: `governance.EvaluationResult{Decision,Reason,Status}` returned by `Evaluate`, mapped to the `{data,error}` envelope (D8). The richer RateLimitInfo/BudgetInfo/UsageInfo sub-structs are ESC-REF-ABSENT (open-questions). |
| PAR-BF-GOV-037 | `BeforeSave` on `TableVirtualKeyProviderConfig` validates `AllowedModels` and `BlacklistedModels` via `Validate()` | `framework/configstore/tables/virtualkey.go:82-90` | HAVE | bf-gov-2: `WhiteList.Validate()`/`BlackList.Validate()` (`lists.go:37,70`) called inline from the VK admin create/update path (`internal/admin/virtualkeys.go:75,78`), rejecting (400) a list that mixes `"*"` with explicit models (D5). GORM `BeforeSave` *mechanism* ESC (no GORM); *validation behavior* built inline. |
| PAR-BF-GOV-038 | `BeforeSave` on `TableRateLimit` validates reset duration format and requires reset duration when max limit is set | `framework/configstore/tables/ratelimit.go:48-87` | HAVE | bf-gov-3: `governance.ValidateRateLimit` (`internal/governance/quota.go`) pure fn — errors when a positive max lacks a reset duration or the duration is unparseable — called LIVE from the VK admin create/update path (`internal/admin/virtualkeys.go`). GORM `BeforeSave` mechanism ESC; behavior built inline (D5). |
| PAR-BF-GOV-039 | `CheckBudget` treats expired budgets (rolling window elapsed) as reset by skipping the check | `plugins/governance/store.go:946-974` | HAVE | bf-gov-3: lazy reset is INHERENT for SQL-live dimensions — the `windowStart(period)` lower bound makes a rolled-over window aggregate only current-period rows (no counter, no `lastReset`), applied to both rate-limit dimensions (D2/D3). |
| PAR-BF-GOV-040 | `CheckRateLimit` checks token and request limits against local usage + remote baseline for multi-node clusters | `plugins/governance/store.go:870` | PARTIAL | matrix-audit (post-bf-gov-4): the LOCAL dual-dimension check (token AND request) IS built and wired live — `QuotaEngine.Evaluate` checks both dimensions via `SumTokensByAPIKey` (`internal/store/requestlog.go:290`) and `SumRequestsByAPIKey` (`:308`) within `windowStart` (`quota.go:297/277`); team RPM via `checkTeamRPM` (`quota.go:227`); config validated by `ValidateRateLimit` (`quota.go:141`). Only the remote/multi-node baseline is DEFERRED (clustering, permanently out of scope). |
| PAR-BF-GOV-041 | `UpdateVirtualKeyBudgetUsageInMemory` walks hierarchy and bumps every applicable budget atomically | `plugins/governance/store.go:1541-1554` | VAR | matrix-audit (post-bf-gov-4): the VK→Team budget HIERARCHY is applied variant-by-design via SQL-LIVE `QuotaEngine.Evaluate` (`internal/governance/quota.go:207`), which evaluates the VK budget and the owning team's budget (`checkTeamBudget` `:315` → `SumCostByTeam`) on every request — no in-memory accumulator to bump. Same VAR family as 013. |
...
| PAR-BF-GOV-044 | `UsageTracker.PerformStartupResets` checks ALL virtual keys (active and inactive) for expired rate limits on startup | `plugins/governance/tracker.go:224-350` | VAR/ESC | bf-gov-3: no in-memory counter exists to reset on startup — every dimension reads `request_log` live, so a fresh process is already correct (no stale in-memory state to repair). Same divergence class as 023 (D4, open-questions). |
| PAR-BF-GOV-045 | `UsageTracker.Cleanup` flushes in-memory deltas to DB before shutdown | `plugins/governance/tracker.go:354-376` | VAR/ESC | bf-gov-3: no in-memory deltas to flush — usage is persisted by `SaveUsage` on each request. Same divergence class as 023 (D4, open-questions). |
...
| PAR-BF-GOV-048 | `TableKey.AfterFind` deserializes `BlacklistedModelsJSON` into `BlacklistedModels` runtime field | `framework/configstore/tables/key.go:662-666` | MISSING (ESC) | bf-gov-2 ESC: same root as 029 — no provider-key table/runtime field in g0router to deserialize into. The in-scope provider-config-level JSON deserialization is handled by the existing `config_json` `json.Unmarshal` (`store/virtualkeys.go:43-52`), no new hook (open-questions). |
