...
|---|---|---|---|---|
| PAR-BF-GOV-001 | Virtual key schema: ID, Name, Value, IsActive, TeamID/CustomerID (mutually exclusive), RateLimitID, CalendarAligned, Budgets, ProviderConfigs, MCPConfigs | `framework/configstore/tables/virtualkey.go:208-241` | MISSING | g0router has `VirtualKey` struct with ID, Name, ProviderConfigs, Budget, RateLimitRPM only: `internal/schemas/governance.go:4-10` |
| PAR-BF-GOV-002 | VK provider config: Provider, Weight, AllowedModels (schemas.WhiteList), BlacklistedModels (schemas.BlackList), AllowAllKeys, Keys (many2many join table), Budgets, RateLimit | `framework/configstore/tables/virtualkey.go:26-40` | MISSING | g0router `ProviderConfig` has Provider, AllowedModels, KeyIDs, Weight only: `internal/schemas/governance.go:13-18` |
...
| PAR-BF-GOV-007 | Team schema: ID, Name, CustomerID, RateLimitID, Budgets, RateLimit, VirtualKeys, VirtualKeyCount (computed), CalendarAligned | `framework/configstore/tables/team.go:12-45` | MISSING | No Team type or table in g0router Go backend |
...
| PAR-BF-GOV-009 | VK ownership mutual exclusion enforced in `BeforeSave`: error if both TeamID and CustomerID non-nil | `framework/configstore/tables/virtualkey.go:260-264` | MISSING | g0router schema has no TeamID or CustomerID fields on VirtualKey |
| PAR-BF-GOV-010 | Hierarchical evaluation order: Provider/Model → User → VK → Team → Customer | `plugins/governance/resolver.go:83-330` | MISSING | g0router has no evaluation resolver |
...
| PAR-BF-GOV-012 | Budget single-owner enforcement in `BeforeSave`: error if more than one owner FK is non-nil | `framework/configstore/tables/budget.go:50-70` | MISSING | g0router Budget has no owner FKs |
...
| PAR-BF-GOV-014 | Budget hierarchy evaluation: VK-level budgets → Team-level budgets → Customer-level budgets via `collectBudgetsFromHierarchy` | `plugins/governance/store.go:977-998` | MISSING | Phase 18A spec documents hierarchical checks: `docs/phases/phase-18-bifrost-features.md:65-69` |
...
| PAR-BF-GOV-020 | Rate limit hierarchy evaluation: VK-level → provider-config-level → Team-level → Customer-level | `plugins/governance/store.go:1522-1538` | MISSING | Phase 18A spec documents hierarchical RPM checks: `docs/phases/phase-18-bifrost-features.md:68-69` |
...
| PAR-BF-GOV-042 | Provider-level budgets and rate limits checked before VK-level in `EvaluateModelAndProviderRequest` | `plugins/governance/resolver.go:83-127` | MISSING | No provider-level governance in g0router |
