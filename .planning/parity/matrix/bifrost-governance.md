...
| PAR-BF-GOV-006 | VK value encrypted at rest: SHA-256 hash for lookup, vault or AES encryption in `BeforeSave` | `framework/configstore/tables/virtualkey.go:260-284` | MISSING | NOTE CORRECTED (matrix-audit post-bf-gov-4): prior "no VK table" text was STALE — a `virtual_keys` table exists (`internal/store/migrate.go:75`). The ENCRYPTION capability is still genuinely absent: the VK bearer value is stored AND looked up in PLAINTEXT (`internal/store/virtualkeys.go:90` INSERT raw `key`; `:132` `WHERE key = ?`) and re-served plaintext to the UI (`internal/admin/virtualkeys.go:31` `Key: vk.Key`); no SHA-256-hash-for-lookup column, no AES/`*_enc` at rest. VIOLATES the AGENTS.md "secrets encrypted at rest via reversible `*_enc`" decision → BUILDABLE-IN-SCOPE, scheduled `bf-gov-5`. Stays MISSING until built. |
