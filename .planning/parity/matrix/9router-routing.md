...
| PAR-ROUTE-005 | Model alias resolution from localDb aliases map | `src/sse/services/model.js:22-25`, `open-sse/services/model.js:182-208` | MISSING | 9router resolves `{alias: "provider/model"}` maps. g0router has no alias table. |
| PAR-ROUTE-006 | Provider alias to ID mapping (~140 aliases) | `open-sse/services/model.js:1-143` | MISSING | Hard-coded `ALIAS_TO_PROVIDER_ID` map. g0router resolves only 3 providers by prefix (`internal/inference/router.go:35-54`). |
| PAR-ROUTE-007 | Provider prefix parsing (`provider/model` or `alias/model`) | `open-sse/services/model.js:155-176` | PARTIAL | g0router parses `anthropic/`, `gemini/`, or defaults to openai (`internal/inference/router.go:35-54`). No alias support. |
| PAR-ROUTE-008 | Model name prefix inference when no alias matches | `open-sse/services/model.js:248-259` | MISSING | 9router infers `claude-`→anthropic, `gpt-`→openai, `o1`/`o3`/`o4`→openai. g0router has no inference fallback. |
...
| PAR-ROUTE-010 | Circular alias loop validation | `docs/ARCHITECTURE.md:17` (mentioned), `.planning/phases/09-models-aliases-combos/PLAN.md:64` | MISSING | 9router plans alias loop detection. g0router Phase 9 plan notes "Alias loops: Validate aliases do not form cycles" as a risk, not implemented. |
...
| PAR-ROUTE-020 | Per-URL retry with configurable attempts/delay by status code | `open-sse/executors/base.js:98-174`, `open-sse/config/runtimeConfig.js:52-57` | MISSING | `DEFAULT_RETRY_CONFIG`: 429→0 attempts, 502→3/3s, 503→3/2s, 504→2/3s. g0router has no retry layer. |
| PAR-ROUTE-021 | Provider-specific retry config override | `open-sse/executors/base.js:105`, `open-sse/config/providers.js:197` | MISSING | e.g. Kiro overrides with `retry: { 429: 2 }`. g0router providers have no retry config. |
| PAR-ROUTE-022 | Connect timeout abort and retry mapping to 502 | `open-sse/executors/base.js:125-128`, `open-sse/executors/base.js:156-163` | MISSING | 60s connect timeout; `AbortError` (non-connect) propagates, connect timeout retries as 502. g0router has no connect timeout handling. |
...
| PAR-ROUTE-044 | Error classification rules (text-first then status) | `open-sse/config/errorConfig.js:59-76` | MISSING | Ordered rules: text substring match, then status match. g0router has no error classification layer. |
| PAR-ROUTE-045 | Precise cooldown override from provider-reported `resetsAtMs` | `src/sse/services/auth.js:211-214`, `open-sse/services/accountFallback.js:49` | MISSING | Provider-specific `resetsAtMs` overrides backoff; capped at 30 min. g0router has no cooldown logic. |
...
| PAR-ROUTE-048 | Quota window parsing (unix timestamps in seconds or milliseconds) | `open-sse/services/usage.js:103-131` | MISSING | Handles both seconds and ms, numeric strings, ISO strings. g0router has no usage/quota parsing. |
