...
| PAR-ROUTE-027 | Weighted provider selection | `.planning/phases/08-keys-virtualkeys-routing/PLAN.md:23` | HAVE | w7-route-b: additive `case "weighted"` in `SelectionEngine.SelectConnection` (`internal/inference/selection.go`); per-connection weight read from `Connection.Metadata` JSON `{"weight":N}` (default 1, no schema change — ESC-WEIGHT-SRC); deterministic smooth weighted round-robin accumulator (no math/rand). Hermetic: `TestWeightedSelectionRatio` (3:1 over 4 selects → cA:3 cB:1), `TestWeightedSelectionDefaultEqual`. |
...
| PAR-ROUTE-035 | Provider-specific URL building with fallback URLs | `open-sse/services/provider.js:155-209`, `open-sse/executors/base.js:20-42` | HAVE | w7-route-b: additive index-based fallback URL list `chatURLs() []string` in `internal/providers/generic/chat.go` (primary at index 0, ordered fallbacks follow); `chatURL()` preserved as `chatURLs()[0]`. Hermetic: `TestGenericChatURLsSingle`, `TestGenericChatURLsFallbackOrder`. Live retry-on-next-URL wiring deferred (ESC-035-RETRY: the request/retry path is in a frozen handler body). |
...
| PAR-ROUTE-039 | Free provider no-auth virtual connection injection | `src/sse/services/auth.js:36-53`, `src/shared/constants/providers.js:14` | HAVE | w7-route-b: `internal/inference/freeconn.go` — `isFreeProvider` sources the noAuth flag from the catalog (`catalog.Lookup(id).NoAuth`, ESC-FREE-SET); `SelectConnection` injects `syntheticFreeConnection` (sentinel id "noauth", empty secret) when a free provider has no real eligible connection (auth.js:36-53). Hermetic: `TestFreeConnInjectedWhenNoStoredConn`, `TestNonFreeProviderNoStoredConnErrors`, `TestSyntheticFreeConnection`, `TestIsFreeProviderFromCatalog`. |
...
| PAR-ROUTE-053 | Project ID cold-miss resolution (antigravity, gemini-cli) | `src/sse/handlers/chat.js:191-198` | HAVE | w7-route-b: `internal/inference/projectid.go` — `ProjectIDManager.EnsureProjectID` resolves (injectable `ProjectIDResolver`) + persists (injectable `ProjectIDPersister`) the project ID on a cold miss for `antigravity`/`gemini-cli`, writing back to `Connection.Metadata` `{"projectId":...}` (no schema change, ESC-PROJ-PERSIST). Wired additively in `AccountRunner.RunModel` (nil manager = no-op). Hermetic: `TestProjectIDColdMissResolvesAndPersists`, `TestProjectIDWarmNoResolve`, `TestProjectIDNonApplicableProvider`, `TestProjectIDNilManager`, `TestProjectIDResolverErrorPropagates`. Production HTTP resolver wiring is integration-only (ESC-PROJ-FETCH, open-questions). |
...
| PAR-ROUTE-055 | Proxy pool resolution per connection (`connectionProxyEnabled`, `vercelRelayUrl`) | `open-sse/handlers/chatCore.js:151-182` | HAVE | w7-plat-1 (PAR-PLAT-009) + w7-route-b verification: per-connection proxy resolution via `platform.ProxyPoolService.ResolveProxyForConnection(conn)` (reads `connection.proxy_pool_id` → active pool → proxy URL) wired into the selection path through `SelectionEngine.ResolveProxy`/`SetProxyResolver` (`internal/inference/selection.go`). g0router's typed `proxy_pool_id` linkage is the architectural equivalent of 9router's `providerSpecificData` proxy config; no ROUTE-055-specific gap (ESC-055-OVERLAP). Hermetic: `TestSelectionResolveProxy`, `TestSelectionResolveProxyNoResolver` (`internal/inference/proxyresolve_test.go`), `internal/platform/proxypools_test.go`. |
| PAR-ROUTE-056 | Live model catalog override (Kiro, Qoder dynamic models) | `src/app/api/v1/models/route.js:16-38`, `src/app/api/v1/models/route.js:289-299` | HAVE | w7-route-b: `internal/inference/livecatalog.go` `LiveCatalogResolver` (injectable `LiveCatalogFetcher` seam, skips upstream connections per 060) + additive `LiveCatalogLister` adapter/`SetLiveCatalogLister` on `ModelsHandler` (`internal/api/models.go`, wired in `routes_openai.go`). Merged after static/custom/alias/sub-config; a resolver error degrades to static-only SILENTLY (route.js:296-298), NOT a 500. Hermetic: `TestLiveCatalogResolverMergesDynamicModels`, `TestModelsList_MergesLiveCatalog`, `TestModelsList_LiveCatalogErrorStaticOnly`, `TestModelsList_LiveCatalogDedup`. Production HTTP fetcher wiring is integration-only (ESC-LIVE-FETCH, open-questions; default nil fetcher = no live call). |
...
| PAR-ROUTE-059 | Web search/fetch model exposure (`{alias}/search`, `{alias}/fetch`) | `src/app/api/v1/models/route.js:386-401` | HAVE | w7-route-b: additive `PseudoModelLister`/`SetPseudoModelLister` on `ModelsHandler` (`internal/api/models.go`); `pseudoModelsAdapter` (`routes_openai.go`) reads connection metadata `providerSpecificData.webSearchConfig`/`webFetchConfig` `{alias,enabled}` and exposes `{alias}/search`+`{alias}/fetch` (no network — exposure only). Hermetic: `TestModelsList_ExposesPseudoModels`, `TestModelsList_NoPseudoModelsWhenUnconfigured`. Live web-search/fetch EXECUTION (reverse-engineered endpoints) is deferred (ESC-WEB-EXEC, open-questions). |
| PAR-ROUTE-060 | Upstream connection detection (UUID suffix) | `src/app/api/v1/models/route.js:46`, `src/app/api/v1/models/route.js:282-284` | HAVE | w7-route-b: pure `IsUpstreamConnection(name)` + `upstreamConnectionRE` (canonical UUID 8-4-4-4-12, anchored to end) in `internal/inference/upstream.go`; gates the live-catalog fetch (056) — upstream connections are skipped in `LiveCatalogResolver.ResolveLiveModels`. Fully deterministic, no I/O. Hermetic: `TestIsUpstreamConnection` (9 cases), `TestLiveCatalogResolverSkipsUpstream`. |
