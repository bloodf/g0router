...
| PAR-BF-OAI-003 | Route `POST /v1/responses` registered with fasthttp | `transports/bifrost-http/handlers/inference.go:725` | HAVE | bf-openai-4 flip (prior MISSING note was STALE): route `r.POST("/v1/responses", responses.Handle)` (`internal/server/routes_openai.go:127`); handler `ResponsesHandler.Handle` (`internal/api/responses.go:56`) translates responses→chat and streams; regression-locked by `TestResponsesEndpointTranslatesRequest` (`internal/api/responses_test.go:146`) + `TestResponsesStreamEventTypingAndDone` (`internal/api/responses_flip_test.go`) |
| PAR-BF-OAI-004 | Route `POST /v1/responses/input_tokens` (count tokens) registered | `transports/bifrost-http/handlers/inference.go:732` | HAVE | bf-openai-4: route `r.POST("/v1/responses/input_tokens", inputTokens.Handle)` (`internal/server/routes_openai.go`); handler `InputTokensHandler.Handle` (`internal/api/input_tokens.go`) translates the responses body → `*ChatRequest` (stream=false, reusing the responses translation path) → `provider.CountTokens`, returns the bare `{"tokens":N}` JSON; openai `CountTokens` implemented over the former stub (`internal/providers/openai/counttokens.go`, POST upstream `/v1/responses/input_tokens`); tests `internal/api/input_tokens_test.go` + `internal/providers/openai/counttokens_test.go` + `TestInputTokensRouteRegistered` |
| PAR-BF-OAI-005 | Route `POST /v1/responses/compact` (compaction) registered | `transports/bifrost-http/handlers/inference.go:733` | MISSING | ESCALATED (bf-openai-4 §8 ESC-COMPACTION): NO `Compaction` method on the `Provider` interface (`internal/schemas/provider.go:69-108`), NO `Compaction`/`Compact*` schema, NO stub. Adding a `Compaction` interface method is non-additive (touches all 43 providers); its wire shape is responses-rewrite-dependent (ESC-REF-ABSENT). STAYS MISSING — see `.planning/parity/plans/open-questions.md` |
...
| ID | Behavior | Evidence (file:line) | g0router status | Notes |
|---|---|---|---|---|
| PAR-BF-OAI-201 | SSE headers set **after** stream setup so provider errors return JSON | `transports/bifrost-http/handlers/inference.go:1652-1666` | HAVE | bf-openai-4 bug fix: the SSE content-type/headers are now set ONLY AFTER the provider stream channel opens successfully; on a stream-open `*ProviderError`, `writeError` runs with the default `application/json` content-type + the provider's real status (`perr.StatusCode`, fallback 502). Applied across every `/v1/*` streaming handler (`internal/api/chat.go`, `responses.go`, `completions.go`, `audio.go` Speech+Transcription, `images.go`, `messages.go`); regression test `TestChatStreamOpenErrorReturnsJSON` + `TestResponsesStreamOpenErrorReturnsJSON` (`internal/api/sse_ordering_test.go`) |
| PAR-BF-OAI-202 | SSE reader bypasses fasthttp internal pipe for one event per chunk | `transports/bifrost-http/handlers/inference.go:1698` (`lib.NewSSEStreamReader`) | MISSING | ESCALATED (bf-openai-4 §8 ESC-SSE-PIPE): pure performance/transport-abstraction; g0router writes directly to `ctx` by design. Not a correctness gap. STAYS MISSING |
| PAR-BF-OAI-203 | SSE event format includes `event: <type>` for Responses and image-gen streams | `transports/bifrost-http/handlers/inference.go:1774,1834` | HAVE | bf-openai-4 flip (responses path): `FormatSSE(FormatOpenAIResponses, …)` emits `event: %s\ndata: %s\n\n` (`internal/translation/sse.go:144-153`); regression-locked by `TestResponsesStreamEventTypingAndDone` (`internal/api/responses_flip_test.go`) asserting `event: response.created`/`response.output_text.delta`/`response.completed`. VARIANT: the image-gen-stream half uses the passthrough processor (plain `data:`, bf-openai-2's domain) — see §8 ESC-IMG-EVENT-TYPE / open-questions.md |
| PAR-BF-OAI-204 | `[DONE]` marker skipped when `includeEventType=true` or `skipDoneMarker=true` | `transports/bifrost-http/handlers/inference.go:1852` | HAVE | bf-openai-4 flip (positive half): the responses stream terminates with `data: [DONE]\n\n` (`internal/translation/stream.go:136`); regression-locked by `TestResponsesStreamEventTypingAndDone` (`internal/api/responses_flip_test.go`). VARIANT-by-design: g0router has no `includeEventType`/`skipDoneMarker` toggle and always terminates with `[DONE]` — see §8 ESC-DONE-SKIP / open-questions.md |
...
| PAR-BF-OAI-208 | Responses streaming supported | `transports/bifrost-http/handlers/inference.go:725` | HAVE | bf-openai-4 flip (prior MISSING note was STALE — the live responses path uses translation, not `provider.Responses`): `ResponsesHandler.Handle` opens `ChatCompletionStream` and pipes through `ProcessTranslateStream(…, FormatOpenAIResponses, …)` (`internal/api/responses.go:132-147`); tested `TestResponsesEndpointStreamsEvents` (`internal/api/responses_test.go:183`) + `TestResponsesStreamEventTypingAndDone` (`internal/api/responses_flip_test.go`) |
...
| ID | Behavior | Evidence (file:line) | g0router status | Notes |
|---|---|---|---|---|
| PAR-BF-OAI-301 | `BifrostError` struct with `event_id`, `type`, `is_bifrost_error`, `status_code`, `error` | `core/schemas/bifrost.go:1687-1696` | MISSING | VARIANT-by-design (bf-openai-4 §1.6): g0router's flat OpenAI `{"error":{message,type,code[,param]}}` + admin `{data,error}` envelope is CANONICAL (AGENTS.md). `event_id`/`is_bifrost_error` are a Bifrost-specific contract, not a g0router gap. STAYS MISSING — envelope NOT restructured |
| PAR-BF-OAI-302 | `ErrorField` struct with `Type`, `Code`, `Message`, `Error`, `Param`, `EventID` | `core/schemas/bifrost.go:1776-1783` | MISSING | VARIANT (bf-openai-4 §1.6): the prior "lacks `Param`" note was STALE — `APIError.Param` ALREADY EXISTS (`internal/schemas/errors.go:7`) and so does `ProviderError.Param` (`errors.go:29`). bf-openai-4 now SURFACES the existing `param` through the error writer (`writeErrorWithParam` in `internal/api/errors.go`; `writeProviderError` forwards `perr.Param`; tests `TestWriteErrorWithParamSurfacesParam`/`TestWriteProviderErrorForwardsParam`). `EventID` is NOT added (variant-by-design — different envelope contract). STAYS MISSING for the full Bifrost `ErrorField`; the `param` half is now satisfied |
| PAR-BF-OAI-303 | Status-code fallback: explicit → 400 for `!IsBifrostError` → 500 default | `transports/bifrost-http/handlers/utils.go:113-119` | PARTIAL | VARIANT-by-design (bf-openai-4 §1.6): g0router sets explicit status in `writeError` (`internal/api/errors.go`) and has no `IsBifrostError` discriminator by design. No change |
| PAR-BF-OAI-304 | SSE error frames send `event: error` or `data: {error}` depending on stream type | `transports/bifrost-http/handlers/utils.go:144-158` | MISSING | The SSE-OPEN error half folds into PAR-BF-OAI-201 HAVE (stream-open errors now return JSON with the correct content-type + real status). The mid-stream `event: error` RE-FRAME is VARIANT (bf-openai-4 §8 ESC-SSE-MIDSTREAM-FRAME): mid-stream errors abort the channel (`internal/translation/stream.go:60-61`) and are logged, not re-framed — the channel is already torn down. STAYS MISSING for the re-frame |
