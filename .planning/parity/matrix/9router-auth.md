...
| PAR-AUTH-017 | Request log header sanitization | `src/lib/db/repos/requestDetailsRepo.js:46-54` | HAVE | SanitizeHeaders in internal/usage/observability.go strips auth headers before request_details persistence (w5-c) |
| PAR-AUTH-018 | Debug log disabled in production | `open-sse/utils/debugLog.js:3` | HAVE | internal/logging.Debug gates on NODE_ENV != production, [DBG:tag] format (w5-c) |
| PAR-AUTH-019 | OAuth credential manager for provider connections | `open-sse/services/oauthCredentialManager.js` (referenced), `src/lib/oauth/services/*.js` | HAVE | g0router implements OAuth with PKCE + refresh + device-code orchestration. **Coverage advanced 3→11 provider flows** (w7-prov-oauth): anthropic/gemini/xai (w3-f) + claude/codex/gemini-cli/qwen/iflow/github/kilocode/cline. Config factories `internal/auth/oauth_providers.go`; device-code path `internal/auth/oauth_device.go`; refresh quirks via additive `OAuthConfig` flags; CredentialResolver (`internal/auth/credentials.go`) drives refresh for the redirect+form/basic/json providers. |
| PAR-AUTH-020 | SSRF protections (outbound proxy, MITM DNS bypass) | `open-sse/utils/proxyFetch.js:314-334`, `src/lib/network/outboundProxy.js` | HAVE | w7-plat-1: `internal/platform/outboundproxy.go` SSRF evaluator (blocks loopback/private/link-local/unspecified/multicast/metadata) guards the connectivity-test probe + user-configured proxy host; ClientPool honors per-instance ProxyURL. MITM DNS-bypass is the w7-plat-3 (mitm) surface; broad outbound-target retrofit is a tracked follow-up. |
