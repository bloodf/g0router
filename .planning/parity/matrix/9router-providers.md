...
| PAR-PROV-012 | **vertex** — Service-account JSON auth, baseUrl dynamically built by `VertexExecutor.buildUrl()`, format `vertex` (native) or `openai` (partner). Static catalog for both vertex and vertex-partner. | `open-sse/config/providers.js:343-352`, `open-sse/config/providerModels.js:580-591`, `open-sse/executors/vertex.js` | HAVE-partial | w7-prov-special-a: **partner-openai path shipped** — `internal/providers/urltemplate` builds `…/projects/{projectId}/locations/global/endpoints/openapi/chat/completions` from `providerSpecificData.projectId`, Bearer auth, partner model block; catalog `vertex` (Format `openai`) + factory dispatch. Covered by `TestBuildURLVertexPartner`/`TestVertexBearerAuthAndSecretSafety`/`TestVertexDispatch`. **ESC-A1: native gemini-on-vertex format + SA-JSON→GCP-token exchange deferred** (operator decision needed on partner-only as full HAVE). |
| PAR-PROV-013 | **minimax / minimax-cn** — API-key auth (x-api-key header), baseUrl `https://api.minimax.io/anthropic/v1/messages` or `https://api.minimaxi.com/anthropic/v1/messages`, format `claude`, URL suffix `?beta=true`. Static catalog (MiniMax-M3, M2.7, M2.5, image model). | `open-sse/config/providers.js:146-154`, `open-sse/config/providerModels.js:340-347`, `open-sse/executors/default.js:52-55` | HAVE | w7-prov-special-a: claude-format via the EXISTING anthropic Messages path (`anthropic.NewForProvider`, catalog base URL + `?beta=true` + `x-api-key` + CLAUDE_API_HEADERS) + factory dispatch by Format `claude`. minimax/minimax-cn catalog entries + minimax model block. Covered by `TestClaudeFormatProvidersDispatch`/`TestNewForProviderClaudeFormat`/`TestClaudeFormatProviders`. |
...
| PAR-PROV-015 | **claude (OAuth alias `cc`)** — OAuth device-code flow, baseUrl `https://api.anthropic.com/v1/messages`, format `claude`, spoofed Claude CLI headers, tokenUrl `https://api.anthropic.com/v1/oauth/token`. Static catalog (claude-opus-4-8 through claude-haiku-4-5). | `open-sse/config/providers.js:51-56`, `open-sse/config/providerModels.js:31-39`, `open-sse/executors/default.js:81-113` | HAVE-partial | w7-prov-oauth: **OAuth flow shipped** — `ClaudeOAuth()` config factory (`internal/auth/oauth_providers.go`, verbatim ref clientId/authUrl/tokenUrl) + flows-map entry `"claude"` (`routes_admin.go`); PKCE-redirect via the existing `OAuthFlow`; refresh via form-POST. Covered by `TestClaudeOAuthConfig`. Catalog/adapter (baseUrl/format/headers/model-catalog) remain the w7-prov-* catalog plans' concern. |
| PAR-PROV-016 | **codex (OAuth alias `cx`)** — OAuth, baseUrl `https://chatgpt.com/backend-api/codex/responses`, format `openai-responses`, headers `originator: codex_cli_rs`, tokenUrl `https://auth.openai.com/oauth/token`. Static catalog with review-model suffix generation. | `open-sse/config/providers.js:70-78`, `open-sse/config/providerModels.js:40-55`, `open-sse/executors/codex.js` | HAVE-partial | w7-prov-oauth: **OAuth flow shipped** — `CodexOAuth()` (verbatim ref clientId/authUrl/tokenUrl + `ExtraAuthParams` id_token_add_organizations/codex_cli_simplified_flow/originator) + flows-map entry `"codex"`; PKCE-redirect; refresh form-POST. Covered by `TestCodexOAuthConfig`. Catalog/adapter (responses API) remain the w7-prov-* catalog plans' concern. |
| PAR-PROV-017 | **gemini-cli (OAuth alias `gc`)** — OAuth, baseUrl `https://cloudcode-pa.googleapis.com/v1internal`, format `gemini-cli`. Static catalog (gemini-3-flash-preview, gemini-3-pro-preview). | `open-sse/config/providers.js:64-68`, `open-sse/config/providerModels.js:56-59` | HAVE-partial | w7-prov-oauth: **OAuth flow shipped** — `GeminiCLIOAuth()` reusing the in-tree gemini clientId+secret + Google authorize/token URLs + `ExtraAuthParams` access_type=offline/prompt=consent + flows-map entry `"gemini-cli"`; redirect auth-code; refresh form-POST with client_secret. Covered by `TestGeminiCLIOAuthConfig`. Catalog/adapter (`gemini-cli` format) remain the w7-prov-* catalog plans' concern. |
| PAR-PROV-018 | **qwen (OAuth alias `qw`)** — OAuth device-code flow, baseUrl `https://portal.qwen.ai/v1/chat/completions`, format `openai`, tokenUrl `https://chat.qwen.ai/api/v1/oauth2/token`, authUrl `https://chat.qwen.ai/api/v1/oauth2/device/code`. Static catalog (qwen3-coder-plus, qwen3-coder-flash, vision-model, coder-model). | `open-sse/config/providers.js:80-85`, `open-sse/config/providerModels.js:60-66`, `open-sse/executors/qwen.js` | HAVE-partial | w7-prov-oauth: **OAuth flow shipped** — `QwenOAuth()` (verbatim ref clientId/deviceCodeUrl/tokenUrl) + flows-map entry `"qwen"`; the NEW additive **device-code + PKCE** path (`StartDevice`/`PollDevice`, `internal/auth/oauth_device.go`) — request-code with S256 challenge + token poll (`grant_type=device_code` + `code_verifier`); refresh form-POST. Covered hermetically by `TestQwenOAuthConfig`/`TestStartDeviceQwen`/`TestPollDevicePendingThenSuccess`/`...SlowDown`/`...Expired/Denied`/`...DeadlineTimeout` (injectable clock, no real sleep). Device admin transport deferred (ESC-DEVICE-ENDPOINT). Catalog/adapter remain the w7-prov-* catalog plans' concern. |
| PAR-PROV-019 | **iflow (OAuth alias `if`)** — OAuth, baseUrl `https://apis.iflow.cn/v1/chat/completions`, format `openai`, tokenUrl `https://iflow.cn/oauth/token`, authUrl `https://iflow.cn/oauth`. Client credentials used in refresh. Static catalog (qwen3-coder-plus, qwen3-max, deepseek-v3.2, kimi-k2, glm-4.7, iflow-rome). | `open-sse/config/providers.js:87-94`, `open-sse/config/providerModels.js:67-83`, `open-sse/executors/default.js:236-246` | HAVE-partial | w7-prov-oauth: **OAuth flow shipped** — `IflowOAuth()` (verbatim ref clientId+secret/authUrl/tokenUrl + `ExtraAuthParams` loginMethod/type=phone) + flows-map entry `"iflow"`; redirect auth-code; the **Basic-auth refresh quirk** via `RefreshMode:"basic"` (clientId:clientSecret header + form). Covered by `TestIflowOAuthConfig`/`TestRefreshBasicAuthMode`. Catalog/adapter remain the w7-prov-* catalog plans' concern. |
| PAR-PROV-020 | **antigravity (OAuth alias `ag`)** — OAuth, baseUrls list with fallback (`daily-cloudcode-pa.googleapis.com` and sandbox), format `antigravity`, headers include `User-Agent: antigravity/1.107.0`. Static catalog calls different backends (gemini, claude, gpt-oss). | `open-sse/config/providers.js:105-113`, `open-sse/config/providerModels.js:84-94`, `open-sse/executors/antigravity.js` | HAVE | **w7-prov-special-b** — `internal/providers/antigravity` executor: v1internal Gemini wire + sandbox fallback URL ordering + UA `antigravity/1.107.0` + `x-request-source: local`. Per-model backend selection (gemini/claude/gpt-oss) via `backendForModel`/the existing converters. PAR-MCP-060 ride-along: `cloakTools` injects the unavailable decoy tools (filter.go). Catalog+9-model block added; factory arm by Format `antigravity`. OAuth token ACQUISITION → w7-prov-oauth (ESC-B3). |
| PAR-PROV-021 | **github (OAuth alias `gh`)** — OAuth, baseUrl `https://api.githubcopilot.com/chat/completions`, responsesUrl `https://api.githubcopilot.com/responses`, format `openai`, extensive Copilot IDE headers (editor-version, copilot-integration-id, x-github-api-version). Static catalog spans OpenAI, Anthropic, Google, Grok, embedding models. | `open-sse/config/providers.js:176-192`, `open-sse/config/providerModels.js:95-126`, `open-sse/executors/github.js` | HAVE-partial | w7-prov-oauth: **OAuth device-code flow shipped** — `GithubOAuth()` (verbatim ref clientId/deviceCodeUrl/tokenUrl, scope read:user) + flows-map entry `"github"`; device-code path (`StartDevice`/`PollDevice`) stores the GitHub access_token. Covered by `TestGithubOAuthConfig` + the shared device-poll tests. **Copilot-token sub-exchange deferred (ESC-GH-COPILOT)** — the `copilotTokenUrl` mint is a follow-up (open-questions.md); no token refresh (`RefreshMode:"none"`). Device admin transport deferred (ESC-DEVICE-ENDPOINT). Catalog/adapter remain the w7-prov-* catalog plans' concern. |
| PAR-PROV-022 | **kiro (OAuth alias `kr`)** — OAuth, baseUrl `https://codewhisperer.us-east-1.amazonaws.com/generateAssistantResponse`, format `kiro`, AWS eventstream headers (`Accept: application/vnd.amazon.eventstream`, `X-Amz-Target`), custom JSON refresh. Static catalog with thinking/agentic variants and strip lists. | `open-sse/config/providers.js:194-206`, `open-sse/config/providerModels.js:127-146`, `open-sse/executors/kiro.js` | HAVE | **w7-prov-special-b** — `internal/providers/kiro` executor + the genuinely-new AWS eventstream binary frame decoder (`eventstream.go`: length/headers/prelude-CRC/payload/message-CRC → parsed event maps, golden wire-byte tested). Reuses the existing `buildKiroPayload`/`kiroToOpenAIResponse` converters via the registry. 12-model block added; factory arm by Format `kiro`. Custom-JSON OAuth token ACQUISITION → w7-prov-oauth (ESC-B3). |
| PAR-PROV-023 | **cursor (OAuth alias `cu`)** — OAuth, baseUrl `https://api2.cursor.sh`, chatPath `/aiserver.v1.ChatService/StreamUnifiedChatWithTools`, format `cursor`, connect+proto headers (`Content-Type: application/connect+proto`). Static catalog (claude-4.5-opus, gpt-5.2-codex, kimi-k2.5). | `open-sse/config/providers.js:208-218`, `open-sse/config/providerModels.js:163-178`, `open-sse/executors/cursor.js` | HAVE | **w7-prov-special-b** (ESC-B1 BUILT — schema soundly reproduced from the frozen ref) — `internal/providers/cursor` executor: connect-RPC frame (1-byte flags + 4-byte BE length + gzip per flag, `frame.go`), hand-rolled protobuf request encode (`encode.go`/`protobuf.go`, explicit FIELD numbers verbatim from `cursorProtobuf.js`) + response decode (`decode.go`), deterministic Jyh-cipher checksum + sha256/uuid-v5 auth headers (`checksum.go`). All golden wire-byte tested. 14-model block added; factory arm by Format `cursor`. OAuth token ACQUISITION → w7-prov-oauth (ESC-B3). |
...
| PAR-PROV-025 | **cline (OAuth alias `cl`)** — OAuth, baseUrl `https://api.cline.bot/api/v1/chat/completions`, format `openai`, tokenUrl `https://api.cline.bot/api/v1/auth/token`, refreshUrl `https://api.cline.bot/api/v1/auth/refresh`, headers built by `buildClineHeaders`. Static catalog (claude-opus-4.7, gpt-5.3-codex, gemini-3.1-pro). | `open-sse/config/providers.js:238-246`, `open-sse/config/providerModels.js:215-224`, `open-sse/executors/default.js:144-145`, `open-sse/executors/default.js:270-290` | HAVE-partial | w7-prov-oauth: **OAuth flow shipped** — `ClineOAuth()` (verbatim ref authorizeUrl/tokenUrl/refreshUrl) + flows-map entry `"cline"`; redirect auth-code with the **base64-code-decode exchange quirk** (`CodeEncoding:"base64-json"`) + the **JSON-body refresh quirk** (`RefreshMode:"json"` to refreshUrl, nested `data` envelope, ISO expiresAt). Covered by `TestClineOAuthConfig`/`TestExchangeBase64JSONCodeEncoding`/`TestRefreshJSONMode`. Catalog/adapter remain the w7-prov-* catalog plans' concern. |
| PAR-PROV-026 | **kilocode (OAuth alias `kc`)** — OAuth, baseUrl `https://api.kilo.ai/api/openrouter/chat/completions`, format `openai`, device-code flow (no refresh token support). Headers include `X-Kilocode-OrganizationID` when `orgId` in providerSpecificData. Static catalog (claude-sonnet-4, gemini-2.5-pro, gpt-4.1, deepseek-chat). | `open-sse/config/providers.js:228-232`, `open-sse/config/providerModels.js:185-194`, `open-sse/executors/default.js:139-143`, `open-sse/executors/default.js:308-311` | HAVE-partial | w7-prov-oauth: **OAuth custom device-auth flow shipped** — `KilocodeOAuth()` (verbatim ref initiateUrl) + flows-map entry `"kilocode"`; the kilocode device variant (`DeviceVariant:"kilocode"`): unauthenticated POST initiate + GET `pollUrlBase/{code}` with status-coded polling (202=pending/403=denied/410=expired/`status==approved`→token); no refresh (`RefreshMode:"none"`). Covered by `TestKilocodeOAuthConfig`/`TestPollDeviceKilocodeStatusCodes`/`TestStartDeviceKilocodeInitiate`. orgId→`Connection.Metadata` is the adapter follow-up (open-questions.md). Device admin transport deferred (ESC-DEVICE-ENDPOINT). Catalog/adapter remain the w7-prov-* catalog plans' concern. |
...
| PAR-PROV-028 | **qoder (alias `qd`)** — No OAuth fields; executor builds full URL itself with `?Encode=1` + sigPath query params. baseUrl kept for introspection only. format `openai`. Static tier + frontier catalog (auto, ultimate, qmodel, dmodel, etc.). | `open-sse/config/providers.js:96-103`, `open-sse/config/providerModels.js:147-162`, `open-sse/executors/qoder.js` | MISSING | **ESC-A3 (w7-prov-special-a — DEFERRED):** the qoder endpoint requires opaque COSY signing (RSA+AES+MD5, ~17 `Cosy-*` headers via `buildCosyHeaders`) + a proprietary body encoder (`qoderEncodeBody`) + a non-trivial custom request shape. The signature algorithm cannot be soundly reproduced from the reference, so per the plan's "DEFER rather than fabricate" rule this single provider is left MISSING. |
...
| PAR-PROV-032 | **azure** — API-key auth, baseUrl empty string (`""`), format `openai`, headers empty. No static catalog in providerModels. | `open-sse/config/providers.js:384-387`, `open-sse/executors/azure.js` | HAVE | w7-prov-special-a: `internal/providers/urltemplate` builds the resource URL `…/openai/deployments/{deployment}/chat/completions?api-version=…` from `providerSpecificData` (azureEndpoint/deployment/apiVersion, deployment falls back to model) + `api-key` auth header. Catalog `azure` (empty BaseURL, Format `openai`, AuthHeader `api-key`); no model block (deployment-driven). Covered by `TestBuildURLAzure`/`TestAzureAuthHeader`/`TestURLTemplateDispatch`. |
| PAR-PROV-033 | **cloudflare-ai** — API-key auth, baseUrl template `https://api.cloudflare.com/client/v4/accounts/{accountId}/ai/v1/chat/completions`, format `openai`. `{accountId}` resolved from `credentials.providerSpecificData.accountId`. Static catalog (Llama, Mistral, DeepSeek, Moonshot, Qwen, FLUX image models). | `open-sse/config/providers.js:390-392`, `open-sse/config/providerModels.js:403-428`, `open-sse/executors/default.js:64-68` | HAVE | w7-prov-special-a: `internal/providers/urltemplate` substitutes `{accountId}` from `providerSpecificData.accountId` (missing = request error) + Bearer auth. Catalog `cloudflare-ai` (template BaseURL) + 24-model block. Covered by `TestBuildURLCloudflare`/`TestBuildURLCloudflareMissingAccount`/`TestChatCompletionRoundTrip`/`TestURLTemplateDispatch`. |
| PAR-PROV-034 | **glm** — API-key auth (x-api-key header), baseUrl `https://api.z.ai/api/anthropic/v1/messages`, format `claude`, URL suffix `?beta=true`. Static catalog (glm-5.1, glm-5, glm-4.7, glm-4.6v). | `open-sse/config/providers.js:131-134`, `open-sse/config/providerModels.js:321-326`, `open-sse/executors/default.js:52-55` | HAVE | w7-prov-special-a: claude-format via the EXISTING anthropic Messages path (`anthropic.NewForProvider`, catalog base URL + `?beta=true` + `x-api-key` + CLAUDE_API_HEADERS) + factory dispatch by Format `claude`. glm catalog entry + model block. Covered by `TestClaudeFormatProvidersDispatch`/`TestClaudeFormatProviders`. (glm-cn / PAR-PROV-035 is the separate openai-format provider.) |
...
| PAR-PROV-036 | **kimi** — API-key auth (x-api-key header), baseUrl `https://api.kimi.com/coding/v1/messages`, format `claude`, URL suffix `?beta=true`. Static catalog (kimi-k2.6, kimi-k2.5, kimi-latest). | `open-sse/config/providers.js:141-144`, `open-sse/config/providerModels.js:334-339`, `open-sse/executors/default.js:52-55` | HAVE | w7-prov-special-a: claude-format via the EXISTING anthropic Messages path (`anthropic.NewForProvider`, catalog base URL + `?beta=true` + `x-api-key` + CLAUDE_API_HEADERS) + factory dispatch by Format `claude`. kimi catalog entry + model block. Covered by `TestClaudeFormatProvidersDispatch`/`TestClaudeFormatProviders`. |
...
| PAR-PROV-040 | **commandcode** — API-key auth, baseUrl `https://api.commandcode.ai/alpha/generate`, format `commandcode`, headers `x-command-code-version`, `x-cli-environment`. Static catalog (deepseek-v4-pro, moonshotai/Kimi-K2.6, zai-org/GLM-5.1, MiniMaxAI/MiniMax-M2.7, Qwen/Qwen3.6-Max-Preview). | `open-sse/config/providers.js:261-267`, `open-sse/config/providerModels.js:446-458`, `open-sse/executors/commandcode.js` | HAVE | w7-prov-special-a: `internal/providers/commandcode` thin adapter reusing the EXISTING converters (`openaiToCommandCodeRequest`/`commandcodeToOpenAIResponse`, registry.go:169-170) — HTTP + registry translate, SSE event stream aggregated to a ChatResponse for the non-stream path. Catalog `commandcode` (Format `commandcode`, `x-command-code-version`/`x-cli-environment` headers) + 11-model block + factory dispatch. Covered by `TestCommandCodeChatCompletion`/`TestCommandCodeChatCompletionStream`/`TestCommandCodeDispatch`. |
...
| PAR-PROV-047 | **xiaomi-tokenplan** — API-key auth, region-based baseUrl resolution (`sgp`, `cn`, `ams`), format `openai`. Static catalog (mimo-v2.5-pro with claude native variant, tts models, voice clone/design). `XiaomiTokenplanExecutor` handles region routing. | `open-sse/config/providers.js:398-401`, `open-sse/config/providerModels.js:551-561`, `open-sse/config/providers.js:447-457`, `open-sse/executors/xiaomi-tokenplan.js` | HAVE | w7-prov-special-a: `internal/providers/urltemplate` resolves region (`sgp`/`cn`/`ams`, default sgp) → baseURL + Bearer auth. Catalog `xiaomi-tokenplan` (sgp seed BaseURL) + 9-model block + factory dispatch. Covered by `TestBuildURLXiaomiTokenplan`/`TestStreamRoundTrip`/`TestURLTemplateDispatch`. **ESC-A2: the `mimo-v2.5-pro-claude` targetFormat:"claude" native variant + tts/voice models port as openai/media entries** (no ModelEntry TargetFormat field). |
...
| PAR-PROV-053 | **deepgram** — API-key auth, baseUrl `https://api.deepgram.com/v1/listen`, format `openai` (STT endpoint). Static catalog (nova-3, nova-2, whisper-large). | `open-sse/config/providers.js:317-319`, `open-sse/config/providerModels.js:785-788` | MISSING | DEFERRED (w7-prov-media ESC-M1). No `/v1/audio/transcriptions` route exists → STT adapter would be dead code; deepgram `/v1/listen` is a non-OpenAI wire. Pick up after a media-transport route lands. |
| PAR-PROV-054 | **assemblyai** — API-key auth, baseUrl `https://api.assemblyai.com/v1/audio/transcriptions`, format `openai`. Static catalog (universal-3-pro, universal-2). | `open-sse/config/providers.js:321-323`, `open-sse/config/providerModels.js:790-793` | MISSING | DEFERRED (w7-prov-media ESC-M1). No STT route; assemblyai is upload-then-poll (async, not hermetically determinable). |
| PAR-PROV-055 | **nanobanana** — API-key auth, baseUrl `https://api.nanobananaapi.ai/v1/chat/completions`, format `openai`. Static catalog (nanobanana-flash, nanobanana-pro image models). | `open-sse/config/providers.js:325-327`, `open-sse/config/providerModels.js:620-623` | MISSING | DEFERRED (w7-prov-media ESC-M2). No `/v1/images/generations` route → image adapter would be dead code; `ImageGeneration` stubbed 501. |
...
| PAR-PROV-058 | **fal-ai** — API-key auth, image-only. Static catalog (flux-schnell, flux-dev, flux-pro, recraft-v3, ideogram-v2, sd-3.5-large). | `open-sse/config/providerModels.js:794-802` | MISSING | DEFERRED (w7-prov-media ESC-M2). No image route; async queue/poll API. |
| PAR-PROV-059 | **stability-ai** — API-key auth, image-only. Static catalog (stable-image-ultra, stable-image-core, sd3.5-large, sd3.5-large-turbo, sd3.5-medium). | `open-sse/config/providerModels.js:803-809` | MISSING | DEFERRED (w7-prov-media ESC-M2). No image route; bespoke multipart wire. |
| PAR-PROV-060 | **black-forest-labs** — API-key auth, image-only. Static catalog (flux-pro-1.1, flux-pro-1.1-ultra, flux-dev, flux-kontext-pro/max with edit capability). | `open-sse/config/providerModels.js:810-817` | MISSING | DEFERRED (w7-prov-media ESC-M2). No image route; async poll API. |
| PAR-PROV-061 | **recraft** — API-key auth, image-only. Static catalog (recraftv3, recraftv2). | `open-sse/config/providerModels.js:818-821` | MISSING | DEFERRED (w7-prov-media ESC-M2). No image route. |
| PAR-PROV-062 | **runwayml** — API-key auth, image/video. Static catalog (gen4_image, gen4_image_turbo, gen4_turbo video, gen3a_turbo video). | `open-sse/config/providerModels.js:822-827` | MISSING | DEFERRED (w7-prov-media ESC-M3). No image route; video has NO interface method (schema frozen); poll API. |
| PAR-PROV-063 | **sdwebui** — API-key auth, image-only. Static catalog (stable-diffusion-v1-5, sdxl-base-1.0). | `open-sse/config/providerModels.js:624-627` | MISSING | DEFERRED (w7-prov-media ESC-M2). No image route; self-hosted runtime base URL. |
| PAR-PROV-064 | **comfyui** — API-key auth, image-only. Static catalog (flux-dev, sdxl). | `open-sse/config/providerModels.js:628-631` | MISSING | DEFERRED (w7-prov-media ESC-M2). No image route; self-hosted graph API. |
| PAR-PROV-065 | **huggingface** — API-key auth, image + STT. Static catalog (flux-1-schnell, sd-xl-base, whisper-large-v3, whisper-small). | `open-sse/config/providerModels.js:632-638` | MISSING | DEFERRED (w7-prov-media ESC-M2). No image/STT route; NOT in the embedding registry — image+STT only. |
| PAR-PROV-066 | **voyage-ai** — API-key auth, embedding-only. Static catalog (voyage-3-large, voyage-3.5, voyage-code-3, voyage-finance-2, voyage-law-2, voyage-multilingual-2). | `open-sse/config/providerModels.js:524-532` | HAVE | w7-prov-media: `internal/providers/voyageai/` real `Embedding` adapter (bearer POST → `…/v1/embeddings`, decode `EmbeddingResponse`) + factory dispatch arm + catalog entry + 7-model `embedding` block + aliases `voyage-ai`/`voyage`. Reachable via `/v1/embeddings` → `provider.Embedding`. |
