...
|---|---|---|---|---|
| PAR-BF-CORE-001 | Provider interface defines 30+ methods covering chat, completion, embedding, image, video, audio, file, batch, cached content, container, and passthrough APIs | `core/schemas/provider.go:590-707` | PARTIAL | `internal/schemas/provider.go:68` has 16 methods; CountTokens subset now SAT via bf-openai-4 (`provider.go:107` + `openai/counttokens.go:23`); missing Rerank, OCR, Video*, Container*, CachedContent*, Passthrough, PassthroughStream, Compaction — all ESC (no funded route; see bf-core-1 §3 ESC list) |
| PAR-BF-CORE-002 | Provider interface accepts `postHookRunner PostHookRunner` and `postHookSpanFinalizer func(context.Context)` on every streaming method | `core/schemas/provider.go:598-606` | HAVE | `internal/schemas/provider.go:76` carries `postHookRunner PostHookRunner` on every stream method (ChatCompletionStream, TextCompletionStream, ResponsesStream, SpeechStream, TranscriptionStream, ImageGenerationStream); `postHookSpanFinalizer` is tracing-ESC (BIFROST-MAP:277). Verified bf-core-1 D5 — no change needed |
...
| PAR-BF-CORE-019 | Fallback chain with `shouldTryFallbacks` and `prepareFallbackRequest` | `core/bifrost.go:4487-4540` | HAVE | variant-HAVE (bf-core-1 D3): g0router has account-level fallback via `selection.go:334` `WithAccountFallback` + `errorclass.go` `Classify` → `ErrorClass`/`Retryable` + `runner.go` `RunModel`; the Bifrost per-attempt-`PreLLMHook` fallback chain (`core/bifrost.go:4487`) is ESC (plugin-pipeline-coupled; BIFROST-MAP:282) |
| PAR-BF-CORE-020 | `AllowFallbacks` field on `BifrostError` controls fallback behavior | `core/schemas/bifrost.go:1683-1699` | HAVE | variant-HAVE (bf-core-1 D3): g0router's fallback decision is Verdict/StatusCode-driven (`errorclass.go:73-113` → `Verdict`; `runner.go:70` reads `pe.StatusCode`; `selection.go:334` `WithAccountFallback` branches on Verdict); an `AllowFallbacks bool` on `ProviderError` would be a dead flag (not wired into any decision path) and is NOT added; the Bifrost `AllowFallbacks`-on-`BifrostError` mechanism is variant-divergent from g0router's Verdict-based fallback engine |
...
| PAR-BF-CORE-027 | Request-type constants: 20+ distinct request types | `core/schemas/bifrost.go:686-706` | PARTIAL | CountTokens request surface now SAT via bf-openai-4 (`provider.go:107` interface method + `openai/counttokens.go:23` impl; RequestType carried as string literal `"count_tokens"` per bf-openai-4 §1.4); the 20+ request-type constant enum is VAR/ESC by design (bf-core-1 D4): g0router uses string literals + dedicated handlers; most enum entries enumerate ESC surfaces (video/container/rerank/ocr/passthrough/compaction) with no funded route — adding dead constants is FORBIDDEN (BIFROST-MAP:392) |
| PAR-BF-CORE-028 | `BifrostContext` with typed key-value storage and request metadata | `core/schemas/bifrost.go:280-286` | PARTIAL | g0router's `GatewayContext` (`provider.go:25-27`) is RequestID-only by deliberate design (bf-core-1 D2): all 20 construction sites + the sole reader (`PostHookRunner.Run`) touch only `RequestID`; the typed-KV/request-metadata surface has no live consumer and is plugin-pipeline + tracing coupled (both ESC, BIFROST-MAP:282/288); no KV/Metadata field added (would be dead — BIFROST-MAP:392); a field is added only by a future LIVE feature that reads it, in the same plan |
...
| PAR-BF-CORE-034 | Semantic cache plugin with dual-path lookup: direct hash + semantic similarity | `plugins/semanticcache/main.go:135-164` | PARTIAL | bf-core-2 D1/D2: the direct-hash path is LIVE (`internal/semcache/cache.go` `Lookup`/`Store` over `sha256(normalized prompt+model)`, `keys.go`) — a flag-gated read-through hook in non-stream chat (`internal/api/chat.go` `SetSemanticCache`) SHORT-CIRCUITS the provider on a hit (proven `internal/server/semcache_wiring_test.go` provider-hits=1, `internal/api/chat_semcache_test.go` chatCalls=0). The SEMANTIC-SIMILARITY half (cosine over embeddings) is DEFERRED — not built — pending the operator embedding-source decision (D2: a nil embedder would be production-inert; ESC-REF-ABSENT blocks the Bifrost shape). Recorded open-questions §bf-core-2 |
| PAR-BF-CORE-035 | Semantic cache config: Provider, EmbeddingModel, TTL, Threshold, Dimension | `plugins/semanticcache/main.go:28-45` | PARTIAL | bf-core-2 D5/D8: TTL via `cache_ttl_seconds` settings key + the `semantic_cache` feature flag (seeded OFF, `internal/store/migrate.go`; `IsFeatureEnabled` `internal/store/featureflags.go`) are the live config surface; `semantic_cache_threshold` (default 0.95) is reserved for the deferred semantic plan. Provider/EmbeddingModel/Dimension are out-of-scope-now with the deferred semantic-similarity half (D2) |
| PAR-BF-CORE-036 | Streaming response accumulation with background reaper and TTL bookkeeping | `plugins/semanticcache/main.go:99-127` | PARTIAL | bf-core-2 D6/D7: TTL bookkeeping + LAZY purge HAVE (`expires_at` column + `PurgeExpiredSemanticCache` called on the read path, `internal/store/semcache.go`; injected clock — hermetic). Streaming response accumulation = VAR/ESC (non-streaming `/v1/chat/completions` only, phase-19:68; the chat hook is in the non-stream branch only). Background reaper goroutine = ESC (lazy purge only; no global state/init per AGENTS.md) |
| PAR-BF-CORE-037 | `VectorStore` abstraction with Ping, CreateNamespace, GetNearest, Add, Delete | `framework/vectorstore/store.go:82-109` | ESC | bf-core-2 §3: g0router uses SQLite + Go-cosine by design (phase-19:61; BIFROST-MAP §281) — NO VectorStore interface. Product-design divergence from Bifrost |
| PAR-BF-CORE-038 | Vector store backends: Weaviate, Redis, Qdrant, Pinecone | `framework/vectorstore/store.go:14-19` | ESC | bf-core-2 §3: external vector backends out of scope by design (phase-19:61). Product-design divergence from Bifrost |
