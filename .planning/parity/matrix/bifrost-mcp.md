...
| PAR-BF-MCP-034 | API: GET /api/mcp/clients (paginated, with VK config merge + OAuth batch fetch) | `transports/bifrost-http/handlers/mcp.go:70`; `95-294` (getMCPClients + getMCPClientsPaginated) | MISSING | NOTE CORRECTED (matrix-audit post-bf-gov-4): g0router HAS a variant MCP management surface — `GET /api/mcp/clients` (`routes_admin.go:178` → `ListClients`, read-only) + `GET/POST/GET/DELETE /api/mcp/instances` (`:180-185`) + VK-MCP-config CRUD (`routes_mcp.go:25-29`). The SPECIFIC bifrost shape (per-CLIENT list with VK-config-merge + per-user OAuth batch-fetch) is absent because per-user/multi-tenant OAuth is permanently DEFERRED. Stays MISSING for the per-client+per-user shape. |
| PAR-BF-MCP-035 | API: POST /api/mcp/client (create with auth branching) | `transports/bifrost-http/handlers/mcp.go:72`; `387-785` (addMCPClient) | MISSING | NOTE CORRECTED (matrix-audit post-bf-gov-4): g0router creates MCP *instances* via `POST /api/mcp/instances` (`routes_admin.go:181` → `CreateInstance`) with OAuth start at `/auth/start`+`/auth/complete`. The bifrost per-CLIENT create-with-auth-branching (per-user header/oauth modes) is absent — per-user/multi-tenant auth is permanently DEFERRED. Stays MISSING for the per-client shape. |
| PAR-BF-MCP-036 | API: PUT /api/mcp/client/{id} (PATCH semantics, immutable fields guarded, redacted merge) | `transports/bifrost-http/handlers/mcp.go:73`; `787-1336` (updateMCPClient) | MISSING | matrix-audit (post-bf-gov-4) CONFIRMED MISSING: g0router has NO in-place MCP instance/client update handler (instances are create/delete-replace; only VK-MCP-config has PUT). The per-client PATCH-with-redacted-merge shape genuinely does not exist. |
| PAR-BF-MCP-037 | API: DELETE /api/mcp/client/{id} (DB-first delete then memory removal) | `transports/bifrost-http/handlers/mcp.go:74`; `1338-1365` (deleteMCPClient) | MISSING | NOTE CORRECTED (matrix-audit post-bf-gov-4): g0router deletes MCP *instances* via `DELETE /api/mcp/instances/{id}` (`routes_admin.go:185` → `DeleteInstance`). The bifrost per-CLIENT delete (DB-first-then-memory two-phase, per-user credential reconciliation) is absent — per-user/multi-tenant is permanently DEFERRED. Stays MISSING for the per-client shape. |
| PAR-BF-MCP-038 | API: POST /api/mcp/client/{id}/reconnect (rejects disabled and per-user types) | `transports/bifrost-http/handlers/mcp.go:75`; `296-334` (reconnectMCPClient) | MISSING | matrix-audit (post-bf-gov-4) CONFIRMED MISSING: g0router has NO MCP reconnect endpoint (grep of `routes_admin.go`/`routes_mcp.go`/`internal/admin/mcp.go` → zero matches); instance re-auth is via `/auth/start`+`/auth/complete`, not a reconnect-with-per-user-rejection handler. Genuinely absent. |
...
| PAR-BF-MCP-080 | ConnectionString stored as *EnvVar (encrypted at rest) | `core/schemas/mcp.go:284` | MISSING (ESC) | matrix-audit (post-bf-gov-4): DOWNGRADED from VAR(narrow) — the D6 reviewer option is exercised. The prior VAR rested ENTIRELY on a DIFFERENT feature's encryption (MCP OAuth secrets `mcpoauth.go:45,49,140 cipher.Encrypt` → `access_token_enc`/`refresh_token_enc`/`verifier_enc`). The actual instance connection fields `url`/`command`/`args`/`env` are stored PLAINTEXT (`internal/store/mcpinstances.go:148`); no `connection_string` column, no `ConnectionString`/`EnvVar` type. The ConnectionString-as-EnvVar-encrypted capability is genuinely absent — capability-VAR was too generous. ESC: additive-buildable if the broader credentials-at-rest hardening (cf. GOV-006 `bf-gov-5`) is extended to MCP instance fields. |
