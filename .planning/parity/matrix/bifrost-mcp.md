...
| PAR-BF-MCP-080 | ConnectionString stored as *EnvVar (encrypted at rest) | `core/schemas/mcp.go:284` | HAVE | bf-mcp-3: MCP instance env secrets now AES-encrypted at rest in `mcp_instances.env_json_enc`; legacy `env_json` drained to `'{}'` on write/backfill and no longer read. `url`/`command`/`args_json` are non-secret and remain plaintext (args excluded from scope — see plan Q1). One-time idempotent backfill on `Open()` (`env_json_enc=''` guard, sibling of the bf-gov-5 VK backfill; no `Open(path, secret)` signature change). Note: stored env is not currently re-fed to the launcher (the single `StartStdio` call uses request-body env), but the row no longer persists plaintext secrets and the session-gated instance DTO sources env via the decrypting store scan. |
