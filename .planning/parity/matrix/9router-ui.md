...
| PAR-UI-132 | g0router-specific auth: `POST /api/auth/setup` first-user creation, `PUT /api/auth/password`, `GET/POST /api/auth/users` | `ui/e2e/mocks/handlers/auth.ts`; `ui/src/components/governance/users-panel.tsx` (w6-k) | EXTRA (in-app user-mgmt subset HAVE — variant) | g0router has user management not in 9router. **w6-k surfaces the in-app subset (`GET/POST /api/auth/users`, `DELETE /api/auth/users/{id}`, `PUT /api/auth/password`) as a Users panel on `/teams` consuming the w6-c-owned `auth.ts` MOCK read-only: HAVE (variant). NO Go user-management endpoints exist — `internal/admin/auth.go` exposes only Login/Logout/Me/Status (w6-k §1.2); serial Go follow-up §8 ESCALATION-2, open-questions. `POST /api/auth/setup` (first-user) belongs to the login/onboarding flow, not w6-k.** |
