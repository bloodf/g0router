...
| PAR-UI-013 | Route `/dashboard/mitm` MITM proxy config | `ui/src/routes/mitm.tsx` | HAVE | w7-plat-3 — real Go backend shipped: `GET /api/mitm/status` ({enabled,tools}), `POST /api/mitm/toggle`, `GET /api/mitm/ca-cert` (raw PEM `application/x-pem-file`, not `{data}`), `POST /api/mitm/tools/{id}`; mock already mirrors the Go DTO (verify-only, no change); `ui/e2e/mitm.spec.ts` green. The live MITM listener + OS-privileged trust-store/hosts-file parts are integration-only / deferred (§1.9 / ESC-OS-PRIV) |
...
| PAR-UI-019 | Route `/dashboard/proxy-pools` proxy pool management with bulk ops | `ui/src/routes/proxy-pools.tsx`, `ui/src/components/platform/proxy-pool-form-modal.tsx`, `ui/src/lib/proxy-pool-form.ts` | HAVE | w7-plat-1: real Go `/api/proxy-pools*` backend lands; the w6-m page (list/create/edit/test/delete) is now wired to real Go DTOs (mock corrected to mirror). Bulk import via `POST /api/proxy-pools/batch`. |
| PAR-UI-020 | Route `/dashboard/skills` agent skills links | `ui/src/routes/skills.tsx`, `ui/src/lib/skills-format.ts` | HAVE | w6-l NEW `/skills` route + w7-mcp-3 real Go backend: `GET /api/skills` → `{data:[skillDTO{name,category,description,url}]}` from a static `skillsCatalog()` (`internal/admin/skills.go`, `TestListSkills`). Page groups by `category`, per-skill copy-to-clipboard; flipped variant-HAVE → true-HAVE (skills.spec green, zero mock change — casing matched). |
...
| PAR-UI-054 | McpMarketplaceModal MCP marketplace | `ui/src/components/mcp/mcp-marketplace-modal.tsx`, `ui/src/lib/mcp-install.ts` | HAVE | w6-l McpMarketplaceModal + w7-mcp-3 real Go backend: browse `GET /api/mcp/clients` (`{data:[clientDTO]}` PascalCase, store ∪ `DefaultPlugins()` — ESC-CLIENT-SRC), install `POST /api/mcp/instances` (url-vs-command, allowlist-gated — PAR-MCP-022/049). `internal/admin/mcp.go`, `TestMcpListClients`/`TestMcpInstanceCRUDStdioHTTPSSE`. Flipped variant-HAVE → true-HAVE (mcp.spec green, zero mock change — PascalCase casing matched). |
...
| PAR-UI-104 | API endpoint: `GET /api/proxy-pools?includeUsage=true` list pools | `ui/src/routes/proxy-pools.tsx`, `ui/e2e/mocks/handlers/proxy-pools.ts` | HAVE | w7-plat-1: real `GET /api/proxy-pools` (bare array under {data}); `?isActive` filter implemented; `?includeUsage` accepted (no-op-safe) — the page does not send it (ESC-USAGE deferred). `internal/admin/proxypools.go` |
| PAR-UI-105 | API endpoint: `POST /api/proxy-pools` create pool | `ui/src/components/platform/proxy-pool-form-modal.tsx`, `ui/e2e/mocks/handlers/proxy-pools.ts` | HAVE | w7-plat-1: real `POST /api/proxy-pools` returns the canonical proxyPoolDTO (201); password masked as password_set. `internal/admin/proxypools.go` |
...
| PAR-UI-112 | API endpoint: `GET /api/tunnel/status` tunnel status | `ui/src/routes/tunnels.tsx`, `ui/e2e/mocks/handlers/tunnels.ts`, `internal/admin/tunnels.go` | HAVE | w7-plat-2 — real Go `GET /api/tunnels` (+ `GET /api/tunnels/health`) backs the w6-m page (path REMAPPED from ref `/api/tunnel/status`; plain REST poll, no SSE). `{data}` snake_case 4-field DTO mirrors the mock |
| PAR-UI-113 | API endpoint: `POST /api/tunnel/enable` / `disable` Cloudflare tunnel | `ui/src/routes/tunnels.tsx`, `ui/e2e/mocks/handlers/tunnels.ts`, `internal/admin/tunnels.go` | HAVE | w7-plat-2 — real Go `POST /api/tunnels/cloudflare` (enable) / `DELETE /api/tunnels/cloudflare` (disable); state machine + injectable runner |
| PAR-UI-114 | API endpoint: `POST /api/tunnel/tailscale-enable` / `disable` Tailscale | `ui/src/routes/tunnels.tsx`, `ui/e2e/mocks/handlers/tunnels.ts`, `internal/admin/tunnels.go` | HAVE | w7-plat-2 — real Go `POST /api/tunnels/tailscale` (enable) / `DELETE /api/tunnels/tailscale` (disable) + the g0router-grouped `/tunnels` page |
