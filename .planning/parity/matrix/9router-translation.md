...
| PAR-TRANS-011 | Claude request translator uses `adjustMaxTokens`: boosts to min 4096 when tools present; ensures `max_tokens > thinking.budget_tokens + 1024` | `open-sse/translator/helpers/maxTokensHelper.js:8-26` | MISSING | g0router `internal/providers/anthropic/converter.go:88-100` uses static `defaultMaxTokens=4096` without tool/thinking adjustments |
...
| PAR-TRANS-016 | OpenAI → Claude adds `cache_control: {type:"ephemeral",ttl:"1h"}` to last system block and last assistant block | `open-sse/translator/request/openai-to-claude.js:91-105`, `open-sse/translator/helpers/claudeHelper.js:94-103` | MISSING | g0router adds no cache_control |
...
| PAR-TRANS-030 | OpenAI → Gemini `tool_choice` maps to `toolConfig.functionCallingConfig.mode` (`auto→AUTO`, `none→NONE`, `function/required→ANY`) | `open-sse/translator/request/openai-to-gemini.js:467-469` (register), helper implied via toolConfig | PARTIAL | g0router `convertToolChoice` (`internal/providers/gemini/converter.go:200-211`) maps `none/function/required` to `NONE/ANY`; defaults to `AUTO` but does not handle `auto` explicitly |
