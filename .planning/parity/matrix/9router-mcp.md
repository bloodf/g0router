...
| PAR-MCP-018 | Cowork settings write `managedMcpServers` to Claude Desktop 3p config | `src/app/api/cli-tools/cowork-settings/route.js:262` | HAVE-SUBSET | w7-mcp-3: PURE `buildManagedServers` assembles `{name,url,transport,oauth,toolPolicy}` from instances (`internal/mcp/toolpolicy.go`, unit-tested `TestBuildManagedServers`). FOOTNOTE: g0router has no Cowork/Claude-Desktop 3p-config file writer (ESC-COWORK-CONFIG / ESC-TOOLPOLICY-SCOPE) — the parity-meaningful builder is ported; no config-file rewrite. |
| PAR-MCP-019 | Managed servers generate `toolPolicy` with bare and prefixed tool names | `src/app/api/cli-tools/cowork-settings/route.js:171` | HAVE-SUBSET | w7-mcp-3: PURE `buildToolPolicy(server,toolNames)` emits both `t` and `{name}-{t}` as `"allow"` (`internal/mcp/toolpolicy.go`, `TestBuildToolPolicyEmitsBareAndPrefixed`). FOOTNOTE: builder ported + tested; no 3p-config writer (ESC-TOOLPOLICY-SCOPE). |
...
| PAR-MCP-022 | Custom plugins support both URL (HTTP/SSE) and command (stdio) modes | `src/app/api/cli-tools/cowork-settings/route.js:179` | HAVE | w7-mcp-3: `CreateInstance` (`internal/admin/mcp.go`) branches on `command` vs `url`: command → allowlist-gated `Launcher.StartStdio` (reject-before-spawn), url+sse → `StartSSE`, url+http → `StartHTTP`. Unit-tested `TestMcpInstanceCRUDStdioHTTPSSE` + reject-before-spawn. |
...
| PAR-MCP-040 | No MCP agent loop | `internal/mcp/` (no agent files) | HAVE | w7-mcp-3: `internal/mcp/agent.go` — bounded multi-turn `Agent.Run` with injected `ModelStep`/`ToolExecutor`, hard `maxTurns` cap (default 8). Tool-execute path real (`ExecuteTool` shares the bridge-backed `ToolExecutor`). Unit-tested: terminate-on-final / feed-back / maxTurns-cap / tool-error-surfaced. The LLM round-trip is integration-only behind the injected `ModelStep` (ESC-AGENT-DEPTH: loop primitive shipped, no live agent HTTP route — the w6-l page has no agent surface). |
| PAR-MCP-041 | No MCP UI pages | `ui/src/` (no MCP pages) | HAVE | UI pages shipped w6-l (variant-HAVE: /mcp, /mcp/tools, McpMarketplaceModal, /skills); w7-mcp-3 supplies the real Go backend, flipping the four surfaces variant-HAVE → true-HAVE. |
...
| PAR-MCP-045 | `buildManagedMcpServers` strips `{name}-` prefixes idempotently | `src/shared/constants/coworkPlugins.js:48` | HAVE | w7-mcp-3: PURE `stripServerPrefix(server,tool)` while-loop removes repeated prefixes (`internal/mcp/toolpolicy.go`, `TestStripServerPrefixIdempotent`); `buildToolPolicy` emits bare + prefixed variants. |
| PAR-MCP-046 | Settings GET strips `{name}-` prefixes when returning plugin tool names | `src/app/api/cli-tools/cowork-settings/route.js:288` | HAVE | w7-mcp-3: the same PURE `stripServerPrefix` (exported `mcp.StripServerPrefix`) applied on the tools-list read in `ListTools` (`internal/admin/mcp.go`). |
| PAR-MCP-047 | Settings GET prefers default `toolNames` over stored `toolPolicy` keys | `src/app/api/cli-tools/cowork-settings/route.js:297` | HAVE | w7-mcp-3: `ListTools` seeds the catalog from `DefaultPlugins().ToolNames` (the default precedence) and dedupes against live-probe names; `buildToolPolicy` derives the bare set otherwise. |
| PAR-MCP-048 | Settings POST sanitizes custom plugin names to `[^a-zA-Z0-9_-]` and truncates to 64 chars | `src/app/api/cli-tools/cowork-settings/route.js:339` | HAVE | w7-mcp-3: PURE `sanitizePluginName` (regex-strip + 64-cap, `internal/mcp/toolpolicy.go`, `TestSanitizePluginNameRegexAnd64Cap`) applied on `CreateInstance` name. |
| PAR-MCP-049 | Settings POST filters custom plugins by allowlist before registration | `src/app/api/cli-tools/cowork-settings/route.js:337` | HAVE | w7-mcp-3: `CreateInstance` command path goes through `Launcher.StartStdio`'s pre-spawn `isAllowedCommand` gate; a non-allowlisted command returns 400 BEFORE any spawn AND before persist (`TestMcpCreateInstanceRejectsNonAllowlistedBeforeSpawn`). |
| PAR-MCP-050 | Settings DELETE resets Cowork config, skip approvals, and cleans legacy 1p entries | `src/app/api/cli-tools/cowork-settings/route.js:396` | HAVE-SUBSET | w7-mcp-3: `DeleteInstance` removes the instance + best-effort `Launcher.Stop(name)` (stops the bridge). FOOTNOTE: the Cowork 3p-config / skip-approvals / 1p-legacy file rewrite is N/A to g0router (no such files — ESC-COWORK-CONFIG). |
...
| PAR-MCP-060 | Antigravity executor hardcodes `mcp_sequential-thinking_sequentialthinking` as unavailable | `open-sse/executors/antigravity.js:433` | HAVE-SUBSET | w7-mcp-3: PURE constant `unavailableAntigravityTool` (`mcp_sequential-thinking_sequentialthinking`, `{description:"This tool is currently unavailable.", unavailable:true}`) included in the `ListTools` output (`internal/admin/mcp.go`). FOOTNOTE: tool-definition ride-along only; the antigravity EXECUTOR is w7-prov-special (ESC-ANTIGRAVITY). |
