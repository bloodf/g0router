# w1-g — OpenAI Responses API translator pair (registry only; endpoint is w1-g2)

Rows: PAR-TRANS-031 (translator-existence clause), PAR-TRANS-032, PAR-TRANS-033 *(as corrected 2026-06-10: clamp lives in Chat→Responses — `clampCallId` defined `request/openai-responses.js:11-13`, applied only at `:265`,`:281`; Responses→Chat passes `item.call_id` unclamped at `:105`)*, PAR-TRANS-034, PAR-TRANS-035 (request directions); PAR-TRANS-036, PAR-TRANS-037, PAR-TRANS-038 (response directions).
...
These translators register `openai-responses:openai` and `openai:openai-responses` (both directions each) in `NewRegistry`, exactly the registrations the ref performs at `request/openai-responses.js:316-318` and `response/openai-responses.js:588-590`. `FormatOpenAIResponses` ("openai-responses") already exists in `formats.go:11`. PAR-TRANS-031's row text is "bidirectional translation exists" — the registered translators close it; the `/v1/responses` route mentioned in the row's gap note is server wiring and ships in follow-up plan **w1-g2** with its own ref citations from the 9router route/handler files.

## Preconditions (grep checks — run before any task; if any fails, IMPL-BLOCKED)
...
4. **Rows 036/037: chat→responses response** (`openai_responses_response.go`) + SSE framing, port `response/openai-responses.js:12-356`:
   ...
   Tests: `TestOpenAIResponsesResponseLifecycle` (created/in_progress once, sequence numbers strictly increasing), `TestOpenAIResponsesResponseTextDeltas` (item added once, deltas, done-on-finish with full text), `TestOpenAIResponsesResponseReasoning` (rs_ item, summary part, deltas, done set), `TestOpenAIResponsesResponseThinkMarkers` (`<think>`/`</think>` in content split into reasoning open/delta/close per ref `:70-84`, ported as-is — this translator handles markers itself), `TestOpenAIResponsesResponseToolCalls`, `TestOpenAIResponsesResponseFlush` (nil chunk closes everything + completed), `TestFormatSSEResponsesEventFraming`.
...
6. **Registration (closes PAR-TRANS-031's translator-existence clause)**. `NewRegistry` adds exactly two `Register` calls, mirroring the ref's four registrations (`request/openai-responses.js:316-318` request directions, `response/openai-responses.js:588-590` response directions): `Register(FormatOpenAIResponses, FormatOpenAI, responsesToOpenAIRequest, responsesToOpenAIResponse)` and `Register(FormatOpenAI, FormatOpenAIResponses, openaiToResponsesRequest, openaiToResponsesResponse)`.
   Tests: `TestNewRegistryWiresResponsesPair` — all four lookups (`RequestTranslatorFor`/`ResponseTranslatorFor` for both pairs) non-nil, plus reflect-pointer identity against the four named functions (same technique as `TestResponseAliasesUseGeminiTranslator`, registry_test.go).

## Binary acceptance criteria (each is a command with a deterministic pass condition)

- `go test ./...` exits 0; `go vet ./...` exits 0.
- `grep -c 'sequence_number' internal/translation/openai_responses_response.go` ≥ 1.
- `grep -rn 'func init(' internal/translation/responses_*.go internal/translation/openai_responses_*.go` → 0 hits.
- `grep -rn 'panic(' internal/translation/responses_*.go internal/translation/openai_responses_*.go` → 0 hits.
- `grep -c 'RequestTranslatorFor(FormatOpenAIResponses, FormatOpenAI)' internal/translation/registry_test.go` ≥ 1 and the four wiring assertions in `TestNewRegistryWiresResponsesPair` pass (covered by `go test`).
- `TestClampCallID` includes a 65-char input asserting `len(result) == 64` and a 64-char input asserting it is returned unchanged (covered by `go test`).
- `grep -n 'POST("/v1/responses"' internal/server/routes_openai.go` → 0 hits (endpoint is w1-g2; this plan must NOT add it).

## Out of scope

`/v1/responses` route + `internal/api/responses.go` handler (w1-g2 — needs its own ref citations from 9router's route/handler files). `convertResponsesApiFormat` (`responsesApiHelper.js:24+`, dead duplicate). Codex executor/auth (`FormatCodex` flows — Wave 2/3). Usage estimation/persistence (Wave 5). RTK/compression. Cursor/Kiro/Ollama formats (w1-h..j). Passthrough-mode Responses event preservation in `ProcessPassthroughStream` beyond what w1-c shipped. Any provider adapter changes (Wave 2).
