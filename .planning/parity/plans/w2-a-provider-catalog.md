...
2. **Model catalogs** (`models.go`), port the `providerModels.js` block for each of the 10 keys VERBATIM:
   - `type ModelEntry struct { ID string; Name string; UpstreamModelID string; Type string; Params []string }` — `Type` ∈ {"", "llm", "embedding", "stt", "image", "tts"} ("" treated as "llm" per ref `(model.type||"llm")`); `UpstreamModelID` defaults to `ID` when the ref omits it; `Params` carries the ref's per-model `params` array verbatim when present (e.g. stt/image/tts entries) to avoid catalog data loss (recorded, unused until Wave 4/5).
   - `var Models = map[string][]ModelEntry{...}` for groq, deepseek, mistral, cohere, together, fireworks, openrouter, xai, perplexity, ollama — each entry (incl. `Type` and `Params`) copied from `providerModels.js` (e.g. deepseek's 6 incl. `deepseek-v4-pro-max`/`-none` with `UpstreamModelID:"deepseek-v4-pro"`; groq's 4 llm + 3 stt with `type:"stt"`; xai's grok-2-image `type:"image"`; mistral-embed/together-embeddings `type:"embedding"`). openrouter: port the STATIC block `providerModels.js:302-320` (7 embedding + 3 tts + 4 image entries — NOT empty). ollama: port the STATIC block `providerModels.js:572-578+` (gpt-oss:120b, kimi-k2.5, glm-5, minimax-m2.5, glm-4.7-flash, qwen3.5, …). cohere: `providerModels.js:508-512` (3 command models).
   ...
   Tests (`models_test.go`): `TestModelsForDeepSeek` (6 entries; `-max`/`-none` UpstreamModelID == "deepseek-v4-pro"), `TestModelTypeDefaulting` (a no-type entry resolves Type "llm" via a `EffectiveType()` helper or stored "llm"), `TestGroqSTTModels` (3 type "stt"), `TestResolveModelUpstream` (alias id → entry with correct UpstreamModelID), `TestOpenRouterCatalogTypes` (embedding+tts+image present with Params), `TestModelsForOllama` (static entries, non-empty), `TestModelsForUnknown` (empty slice).
