...
- `grep -c '_kiroUpstreamModel' internal/translation/openai_kiro_request.go` → exactly 0 in emitted payload keys (the citation comment may name it; the assertion is `TestKiroPayloadEnvelope`'s no-key check, covered by go test).
