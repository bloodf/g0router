...
Rows: PAR-TRANS-002 (ollama + commandcode format clauses), PAR-TRANS-047 (NDJSON closure note — parser already shipped in w1-c; this plan adds the translators that consume it).
...
- `open-sse/utils/ollamaTransform.js` (1-85)
...
NEW: `internal/translation/openai_ollama_request.go` + `_test.go`, `ollama_openai_response.go` + `_test.go`, `ollama_transform.go` + `_test.go`, `openai_commandcode_request.go` + `_test.go`, `commandcode_openai_response.go` + `_test.go`.
...
3. **openai→ollama lines transform** (`ollama_transform.go`), port `ollamaTransform.js` transform core as PURE functions (the HTTP/TransformStream shell is endpoint wiring, later wave): `OpenAIChunkToOllamaLines(chunk map[string]any, state *StreamState) []map[string]any` — content → `{model, message:{role:assistant, content}, done:false}`; tool_calls accumulate per index (name+arguments string concat, :29-38); finish_reason `tool_calls`/`stop` with pending calls → done:true line with `tool_calls` (arguments parsed, malformed → `{}` per :52 catch), else stop → plain done line (:45-65); plus `OllamaDoneLine(model string)` for the `[DONE]`/flush line (:17-20, 72-75). StreamState: `OllamaPendingToolCalls map[int]map[string]any`.
   Tests: `TestOllamaTransformContentLine`, `TestOllamaTransformToolCallAccumulateAndFlush` (malformed args → empty object), `TestOllamaTransformStopWithoutTools`, `TestOllamaTransformDoneLine`.

4. **openai→commandcode request** (`openai_commandcode_request.go`), port `openai-to-commandcode.js:16-168`: `flattenText` (:16-28), `toContentBlocks` (:30-51 — images become `{type:text, text:"[image omitted]"}`), `safeParseJson` (:53-57 — malformed → `{}`), `convertMessages` (:59-110 — system texts join "\n\n" into top-level STRING param; tool → tool-result block `{toolCallId, toolName, output:{type:text, value}}`; assistant → text + tool-call blocks `{toolCallId, toolName, input}`; default branch → user with content blocks), `convertTools` (:112-132 — both OpenAI and Claude tool shapes → `{name, description, input_schema}`). Envelope (:134-168): `threadId` = random UUID (`uuid.NewRandom`, wrapped error — no panic), `memory:""`, `config` with `workingDir` = `os.Getwd()` (error → ""), `date` = `time.Now().UTC().Format("2006-01-02")`, `environment` = `runtime.GOOS`, empty structure/git fields verbatim; `params` with defaults `max_tokens` 64000 (after `max_tokens` else `max_output_tokens`), `temperature` 0.3, `stream: stream != false` semantics → Go: the bool as passed; `system` only when non-empty; `top_p` when non-nil.
