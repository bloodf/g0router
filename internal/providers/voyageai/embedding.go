...
// Embedding sends an embedding request to voyage-ai. It mirrors
// openai/embedding.go: a bearer-auth POST of the OpenAI-shaped EmbeddingRequest
// body, decoded straight into EmbeddingResponse (reference normalize is
// identity). The endpoint is the full catalog BaseURL (…/v1/embeddings).
func (p *Provider) Embedding(ctx *schemas.GatewayContext, key schemas.Key, request *schemas.EmbeddingRequest) (*schemas.EmbeddingResponse, *schemas.ProviderError) {
	...
	req.SetRequestURI(p.embeddingsURL())
