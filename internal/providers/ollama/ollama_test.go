...
func TestNewBuildsOllamaOpenAICompatibleProvider(t *testing.T) {
	var gotPath string
	var gotAuth string
	...
	if provider.Name() != providers.ProviderOllama {
...
func testKey() providers.Key {
	return providers.Key{Value: "test-key", Provider: providers.ProviderOllama, ConnID: "conn-1", AuthType: "api_key"}
}
