...
func TestNewBuildsCohereOpenAICompatibleProvider(t *testing.T) {
	var gotPath string
	var gotAuth string
	...
	if provider.Name() != providers.ProviderCohere {
...
func testKey() providers.Key {
	return providers.Key{Value: "test-key", Provider: providers.ProviderCohere, ConnID: "conn-1", AuthType: "api_key"}
}
