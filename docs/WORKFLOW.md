...
current_stage: "BIFROST BUILDABLE SET COMPLETE + CREDENTIALS-AT-REST HARDENING. Prior sessions: bf-openai 1–4, bf-gov-1/2/3/4, bf-mcp-1/2, bf-core-1/2, bf-mcp-sat. This session (2026-06-16): a 2-lane matrix-honesty audit (1cb77ad — GOV-013/041→VAR, GOV-040→PARTIAL, MCP-080→MISSING/ESC + note fixes), then two AUDIT-FOUND buildable gaps shipped — bf-gov-5 (1240e57, GOV-006 VK value encrypted at rest) and bf-mcp-3 (0e5188f, MCP-080 MCP instance env encrypted at rest). All on main, strict TDD, adversarial orchestrator review (bf-gov-5 review caught a SumCostByTeam silent-zero blocker). NEXT: the bf-followups phase below (planned, not started)."
...
last_completed_plan: "bf-mcp-3 (MCP instance env encrypted at rest, PAR-BF-MCP-080 → HAVE, commit 0e5188f). Built in an isolated worktree, independently verified by the orchestrator (gate green, 6 load-bearing tests), then fast-forward-merged. Final gate green: go build/vet/test ./... (44 pkgs)."
...
Responses extras + SSE correctness + error-field reconciliation. FINAL plan in the OpenAI serial chain (bf-openai-1 → 2 → 3 → 4); the `internal/server/routes_openai.go` serial slot is released with no successor — the OpenAI chain is COMPLETE.
- **P0/base.** `<base>` = `35c8c0842a6b78c407e95e317fc31fcebd9fc52d`. Untouched-green baseline: `go build/vet/test ./...` exit 0 (1887 tests). Pre-existing dirty `ui/dist/index.html` (built artifact) left untouched (explicit `git add <file>` only) — same observation as bf-openai-1/2/3.
- **BUILD — PAR-BF-OAI-004 (`/v1/responses/input_tokens` CountTokens) → HAVE.** New openai provider impl `internal/providers/openai/counttokens.go` (over the former `stubs.go` 501) posts to upstream `/v1/responses/input_tokens` and decodes `{input_tokens|tokens: N}` → `*TokenCountResponse`; `RequestType "count_tokens"` (string literal, no new catalog constant). New handler `internal/api/input_tokens.go` (`InputTokensHandler`) reuses the SHIPPED responses→chat translation (`stream=false`) → resolve → x-g0-vk gate + pinned override → `provider.CountTokens` → bare `{"tokens":N}` JSON (NO admin envelope). Route `r.POST("/v1/responses/input_tokens", inputTokens.Handle)` wired in the serial slot, coexisting with the static `/v1/responses` route. **Upstream-count-body decision (ESC-COUNT-BODY): Option (a)** — OpenAI's tokenizer is model-keyed/body-shape-tolerant, so the chat-shaped translated body is posted directly; hermetic round-trip test proves it (no interface change, no fall to Option B/501).
- **BUILD — PAR-BF-OAI-201 (SSE-setup-ordering bug) → HAVE.** Real bug fixed: every `/v1/*` streaming handler set `text/event-stream` BEFORE confirming the provider stream opened (and hardcoded `StatusBadGateway` on stream-open error). Fix (additive/surgical): open the stream FIRST; on `*ProviderError` call `writeError`/`writeProviderError` with the default `application/json` content-type + the provider's real status (`perr.StatusCode`, fallback 502, mirroring the non-stream branch); set SSE headers ONLY after a successful open. Applied to `chat.go`, `responses.go`, `completions.go`, `audio.go` (Speech+Transcription), `images.go`, `messages.go`. Regression test `internal/api/sse_ordering_test.go`. (`writeError` already resets the content-type to JSON, so the load-bearing observable is the corrected status code.)
- **FLIP — PAR-BF-OAI-003/208/203/204 → HAVE (regression-locked, no feature code).** Discovery: four matrix rows were STALE-MISSING — the live tree already had `POST /v1/responses` (003, `routes_openai.go:127` + `responses.go:56`), responses streaming (208, `responses.go:132-147`), `event: <type>` typing (203 responses half, `sse.go:144-153`), and `[DONE]` (204 positive half, `stream.go:136`). Added `TestResponsesStreamEventTypingAndDone` (`internal/api/responses_flip_test.go`) to lock them; flipped to HAVE with cites.
- **VARIANT — PAR-BF-OAI-301/302/303/304.** The `{data,error}`/flat OpenAI error envelope is CANONICAL (AGENTS.md) — NOT restructured. The matrix:108 "lacks Param" cite was STALE: `APIError.Param` already exists (`internal/schemas/errors.go:7`). One tiny additive shipped: `writeErrorWithParam` sibling (`internal/api/errors.go`; `writeError` delegates with nil — all ~80 call sites stable) surfaces a non-nil `param`, and `writeProviderError` forwards `perr.Param` (tests in `internal/api/errors_test.go`). `event_id`/`is_bifrost_error` NOT added (variant-by-design). 301/303 stay MISSING/PARTIAL by design; 302 records the `param`-surface variant-augment; 304's open-error half folds into 201, the mid-stream re-frame stays MISSING (ESC-SSE-MIDSTREAM-FRAME). The 203 image-gen half (ESC-IMG-EVENT-TYPE) and 204 skip-toggle (ESC-DONE-SKIP) stay MISSING by design.
- **ESCALATE — PAR-BF-OAI-005/202/205 → STAY MISSING.** 005 (`/v1/responses/compact`): NO `Compaction` interface method (`provider.go:69-108` grep-confirmed absent — MAP premise FALSE), NO schema, NO stub; non-additive (43 providers) + responses-rewrite-dependent (ESC-COMPACTION). 202 (fasthttp pipe-bypass reader): perf/transport-abstraction (ESC-SSE-PIPE). 205 (raw-bytes passthrough): cross-cutting transport (already MAP-ESC). NONE built; appended to open-questions.md.
- **Gates GREEN + HERMETIC.** `go build ./... && go vet ./... && go test ./...` exit 0 (1903 tests). Targeted `CountTokens|Responses|SSE|InputTokens|WriteError|WriteProviderError` (api+openai) all pass; `NotImplemented` (openai) still passes — the remaining `Responses`/`ResponsesStream` stubs stay 501 (UNTOUCHED). `routes_openai.go` touched in exactly ONE commit, additive (zero deletions). NO `internal/schemas/*` edit; NO bf-core-1 micro-serial coordination needed.
- **Closeout.** Matrix `bifrost-openai.md` flipped (004/201/003/208/203/204 → HAVE; 005/202/205/301/302/304 → MISSING with escalate/variant notes; 303 → PARTIAL variant). **FINAL bifrost-openai state: 31 HAVE / 58 MISSING / 1 PARTIAL (+1 EXTRA), 91 rows.** The remaining MISSING are out-of-scope (video 012-017, files-detail/normalization 101-118, governance 401-405/501-507, etc.) — not bf-openai-4's surface. NOT committed: `.planning/parity/plans/bf-openai-4.md`, `ui/dist/index.html`. SERIAL SLOT released with no successor — OpenAI chain COMPLETE.

## bf-gov-1 (closeout) - 2026-06-15
VK↔Team 2-level governance hierarchy (Go-only; NO new routes, NO UI, NO admin route block). First holder of the `internal/governance/quota.go` internal-serial slot in the bf-gov-1 → bf-gov-3 chain; the slot is RELEASED to bf-gov-3 on this close commit. ESC-REF-ABSENT honored — built to the matrix + g0router conventions only, never to a guessed Bifrost wire format.
- **P0/base.** HEAD = `aaee31a` (bf-openai chain complete), branch `main`. Pre-existing dirty `ui/dist/index.html` (built artifact) left UNTOUCHED (explicit `git add <file>` only; never `-A`) — same observation as the bf-openai chain.
- **BUILD — VK→Team link (PAR-BF-GOV-001/007/D4).** Additive `virtual_keys.team_id TEXT NOT NULL DEFAULT ''` via the `ensureColumn` loop (`migrate.go`); `store.VirtualKey.TeamID` threaded through INSERT/UPDATE/SELECT/scan as a queryable column (NOT config_json); `schemas.VirtualKey.TeamID` additive field; team_id settable via the VK admin DTO/handlers. Empty string = un-teamed (Team tier skipped). `IsActive` reconciled to the existing NOT-NULL-DEFAULT-1 column (D1, VAR=nil-means-active — no Go `*bool` churn).
- **BUILD — live Team-budget aggregate (PAR-BF-GOV-014/D8).** `store.SumCostByTeam(teamID, sinceISO)` sums `request_log.cost` over every VK on the team (subquery join through `virtual_keys.team_id`), mirroring `SumCostByAPIKey` (sql.NullFloat64 → 0). This is the LIVE enforcement source — the display-only `teams.budget_used_usd` is NOT enforced against (it has no live writer in the request path; grep-verified).
- **BUILD — 2-level hierarchy (PAR-BF-GOV-010/020/042/D3).** `SumCostByTeam` added to the `SpendReader` interface; Team* fields added to `governance.VirtualKeyInfo`; `checkTeamBudget` (live SumCostByTeam vs `TeamBudgetLimit`, guard >0) + `checkTeamRPM` (synthetic `team:<id>` in-memory rpm window) wired into `Allow` AFTER the VK checks (fail-closed precedence: VK budget → VK RPM → Team budget → Team RPM). `NewQuotaEngine`/`Allow` signatures preserved (interface gains a method only). Denial reasons: "team budget exhausted" / "team rate limit exceeded" (429). End-to-end server test proves a teamed VK whose aggregate request_log spend exceeds `budget_usd` is denied while its own budget passes; an un-teamed VK is allowed (Team tier skipped).
- **BUILD — AllowAllKeys (PAR-BF-GOV-002/D5).** `schemas.ProviderConfig.AllowAllKeys` + `api.VKProviderConfig.AllowAllKeys` consumed in `AllowVK`'s keyIDs branch: true ⇒ no pinned keyIDs (fall-through to normal selection). false+empty KeyIDs ⇒ fall-through default (no invented 403; deny-semantics recorded in open-questions, ESC-REF-ABSENT).
- **BUILD — inline budget single-owner validation (PAR-BF-GOV-012/D2).** `governance.ValidateBudgetOwner(BudgetOwner{VirtualKeyID,TeamID})` errors when >1 of {VK,Team} owner is set; consumed live in the VK admin validation path (`internal/admin/virtualkeys.go:93`). Replaces Bifrost's GORM `BeforeSave` (ESC mechanism) with an inline value-in/error-out check.
- **HAVE-by-design — ownership XOR (PAR-BF-GOV-009/D6).** With CustomerID ESC, TeamID is the only VK owner, so TeamID-XOR-CustomerID holds by construction; a future Customer-tier plan owns reinstating the XOR.
- **Gates GREEN + HERMETIC.** `go build ./... && go vet ./... && go test ./...` exit 0 (1914 tests, 43 packages). Strict TDD: every `_test.go` committed RED before its impl (red→green cadence per §4). All quota tests use the injectable `clock`/`fixedClock` + extended in-memory `fakeSpendReader.SumCostByTeam` — NO real time.Now, NO sleep, NO network, NO subprocess (D7). Store/server tests use temp/in-memory SQLite via `store.Open`. Additive migration only (no DROP/RENAME). No `init()`, no `sync.Map`/CAS/GORM hook, no Customer table.
- **Matrix flip (`bifrost-governance.md`).** 001 → PARTIAL (VK→Team/IsActive HAVE; CustomerID ESC); 002 → PARTIAL (AllowAllKeys HAVE; BlacklistedModels → bf-gov-2; many2many ESC); 007 → HAVE (budget+RPM subset); 009 → HAVE (by-design); 010 → PARTIAL (VK→Team slice); 012 → HAVE (inline validation); 014 → PARTIAL (VK→Team budget); 020 → PARTIAL (VK→Team RPM); 042 → PARTIAL (VK→Team precedence).
- **Closeout.** ESC items + D-deferred follow-ups appended to `open-questions.md`. NOT committed: `.planning/parity/plans/bf-gov-1.md`, `ui/dist/index.html`. quota.go internal-serial slot RELEASED to bf-gov-3. bf-gov-2 (lists.go) runs ∥ (disjoint).

## bf-gov-2 (closeout) - 2026-06-15
Typed WhiteList/BlackList allow/block-list semantics + provider-config-level blacklist (blacklist-wins 2-pass) in the VK model gate (Go-only; NO new routes, NO UI, NO migration/column; `internal/governance/quota.go` UNTOUCHED — bf-gov-2 holds NO internal-serial slot and runs ∥ bf-gov-1). ESC-REF-ABSENT honored — built to the matrix + g0router conventions only.
- **P0/base.** HEAD = `1247fad` (bf-gov-1 shipped+merged), branch `main`. Pre-existing dirty `ui/dist/index.html` (built artifact) left UNTOUCHED (explicit `git add <file>` only; never `-A`) — same observation as the bf-gov-1/bf-openai chains.
- **BUILD — typed list types (PAR-BF-GOV-026/027 + PAR-BF-OAI-119/D1/D2/D6).** NEW pure leaf `internal/schemas/lists.go`: `WhiteList []string` with `IsAllowed` (`["*"]` allows all, empty denies all, else membership) and `BlackList []string` with `IsBlocked` (empty blocks none, `["*"]` blocks all, else membership — empty checked FIRST so an unconfigured blacklist never blocks). The block decision is a METHOD on the type (closes OAI-119). No store/governance/api import, no `init()`, no global state. Wildcard sentinel is the literal `"*"` (no glob).
- **BUILD — additive provider-config blacklist (PAR-BF-GOV-030/D4).** `schemas.ProviderConfig.BlacklistedModels schemas.BlackList \`json:"blacklisted_models,omitempty"\`` (`governance.go:17`) rides the existing `config_json` blob — NO new column, NO migration; `omitempty` keeps pre-bf-gov-2 blobs byte-stable (absent ⇒ nil ⇒ block-none). `api.VKProviderConfig.BlacklistedModels` + the additive `storeVKToAPI` copy line (`routes_openai.go:200`) thread it to the gate.
- **BUILD — blacklist-wins 2-pass gate (PAR-BF-GOV-028/D3).** `matchProviderConfig` (`internal/api/vk.go:116-124`) runs the blacklist pass FIRST (a model in `BlacklistedModels` skips the config) then the allowlist pass; a model in BOTH lists is denied 403 (blacklist wins). The GATE deliberately KEEPS legacy match-all on empty `AllowedModels` (nil OR empty) for backward-compat (VAR, D1) — it does NOT adopt the type-level empty=deny-all, so no existing VK is silently 403'd; the non-empty allowlist restricts via `WhiteList.IsAllowed`. `AllowVK`/`NewVKGate`/`matchProviderConfig` signatures preserved. Reuses the existing 403 "provider/model not allowed for virtual key" reason (no new status/envelope).
- **BUILD — inline list validation (PAR-BF-GOV-037/D5).** `WhiteList.Validate()`/`BlackList.Validate()` (reject a list mixing `"*"` with explicit models) called inline from the VK admin create/update path (`internal/admin/virtualkeys.go:75,78`); invalid list ⇒ 400. Live no-leftovers consumer (grep-proven). GORM `BeforeSave` mechanism ESC.
- **Gates GREEN + HERMETIC.** `go build ./... && go vet ./... && go test ./...` exit 0 (1953 tests, 43 packages). Strict TDD: every `_test.go` committed RED before its impl (red→green cadence per §4; TDD-order proof passes). lists.go is pure (table-driven over `["*"]`/empty/listed hit+miss + blacklist-wins cross case); gate/admin tests reuse existing fakes + temp SQLite — NO time.Now, NO sleep, NO network, NO subprocess (D7).
- **Matrix flip.** `bifrost-governance.md`: 026 → HAVE (WhiteList type implements matrix contract incl. empty=deny-all; gate preserves legacy match-all VAR); 027 → HAVE (BlackList empty=block-none); 028 → HAVE (blacklist-wins 2-pass); 029 → MISSING (ESC — no provider-key model store); 030 → HAVE (config-level blacklist, rides config_json); 037 → HAVE (inline validation); 048 → MISSING (ESC — same root as 029). `bifrost-openai.md`: 119 → HAVE (IsBlocked is a method on BlackList).
- **Closeout.** ESC items (029/048) + D-deferred follow-ups appended to `open-questions.md`. NOT committed: `.planning/parity/plans/bf-gov-2.md`, `.planning/parity/plans/bf-gov-3.md`, `ui/dist/index.html`. quota.go UNTOUCHED (no serial slot held).

## bf-gov-3 (closeout) - 2026-06-15
Dual-dimension rate limit (token + request, BOTH SQL-live over `request_log`) + calendar-aligned lazy reset (reusing the shipped `windowStart`, inherent in the bounded aggregate) + `Decision`/`EvaluationResult` enum surfaced through the gate `error.code` + inline `ValidateRateLimit` (Go-only; NO new routes, NO UI, **NO new table, NO server change**). SECOND and FINAL holder of the `internal/governance/quota.go` internal-serial slot (bf-gov-1 → bf-gov-3); the slot is RELEASED to NOBODY on this close (chain END). ESC-REF-ABSENT honored — built to the matrix + g0router conventions only.
- **P0/base.** HEAD = `be62896` (bf-gov-2 shipped+merged), branch `main`. Pre-existing dirty `ui/dist/index.html` (built artifact) left UNTOUCHED (explicit `git add <file>` only; never `-A`).
- **BUILD — SQL-live dual-dimension aggregates (D1/D3).** `Store.SumTokensByAPIKey` (`SUM(prompt_tokens+completion_tokens)`) + `Store.SumRequestsByAPIKey` (`COUNT(*)`), each bounded by `timestamp >= windowStart(period)`, additive on `requestlog.go` + the `SpendReader` iface. NO in-memory counter, NO CAS, NO accrual, NO sync worker, NO new table/column (`migrate.go` unchanged); restart-survival automatic, lazy reset inherent in the windowStart-bounded WHERE clause.
- **BUILD — Decision enum + Evaluate (D8).** `governance.Decision` typed enum + `EvaluationResult{Decision,Reason,Status}` + the new `Evaluate` method; the shipped `Allow(bool,int,string)` RE-EXPRESSED as a thin wrapper over `Evaluate` (signature PRESERVED — every existing caller + quota test unchanged). Surfaced LIVE through `vkQuotaAdapter.Allow` (calls `Evaluate`) → the gate `error.code` via `api.DecisionCodeForReason` (e.g. a `TokenLimited`/`RequestLimited` denial reaches the chat response with `error.code` `token_limited`/`request_limited`).
- **BUILD — calendar reset reuse (D2).** REUSED the shipped `windowStart` daily/weekly/monthly + injectable clock; only the `default` branch EXTENDED to parse rolling tokens (1h/1d/1M) — the 3 calendar cases UNCHANGED. `ValidateRateLimit` rejects an unparseable token at config time.
- **BUILD — inline validation (D5).** `governance.ValidateRateLimit` pure fn (positive max requires a parseable reset duration), called LIVE from the VK admin create/update path (`internal/admin/virtualkeys.go`). Dual-dimension config rides the existing VK `config_json` blob (`schemas.RateLimit`, additive `omitempty`) — NO new column.
- **VERIFY — streaming finalization (021/022 VAR).** `SaveUsage` is the streaming-aware finalized write (one `request_log` row per completed request with final usage); verified via `internal/api/usage_glue_test.go:721`. NO `UsageUpdate` struct, NO `Accrue` method built (D4).
- **Matrix flip (`bifrost-governance.md`).** 015 → HAVE; 016 → VAR/ESC; 017 → PARTIAL; 018 → PARTIAL (SQL-accrual); 019 → HAVE; 021 → VAR; 022 → PARTIAL; 023 → VAR/ESC; 035 → HAVE; 036 → PARTIAL; 038 → HAVE; 039 → HAVE; 044 → VAR/ESC; 045 → VAR/ESC. The 016/023/044/045 rows are VAR/ESC by-design (fully SQL-live governance — no in-memory accumulator → no sync worker).
- **Closeout.** ESC/VAR items + D-deferred follow-ups appended to `open-questions.md`. NOT committed: `.planning/parity/plans/bf-gov-3.md`, `ui/dist/index.html`. `internal/server/server.go` + `migrate.go` UNTOUCHED (0-byte diff). quota.go internal-serial slot CLOSED (chain END, released to nobody). bf-gov-2 (lists.go) ran ∥ (disjoint).

## bf-core-1 (closeout) - 2026-06-15
Provider-interface reconciliation closed as **matrix-flip-only** (NO Go file changed; NO additive code with a live consumer).
- **SAT (already shipped by bf-openai-4).** CountTokens reconciliation (PAR-BF-CORE-027 subset): interface method `provider.go:107` + openai impl `openai/counttokens.go:23` — verified present at `<base>` = ad0cb82; no code added by bf-core-1.
- **HAVE (already correct).** PAR-BF-CORE-002: `postHookRunner PostHookRunner` on every stream method (`provider.go:76`); `postHookSpanFinalizer` is tracing-ESC (BIFROST-MAP:277). No change.
- **variant-HAVE.** PAR-BF-CORE-019: g0router account-level fallback is the working mechanism (`selection.go:334` `WithAccountFallback` + `errorclass.go` `Classify` + `runner.go` `RunModel`); per-attempt PreLLMHook chain is ESC (plugin-coupled).
- **variant-HAVE.** PAR-BF-CORE-020: fallback decision is Verdict/StatusCode-driven; `AllowFallbacks` flag on `ProviderError` would be dead (not wired into any decision path) and is NOT added; Bifrost mechanism is variant-divergent.
- **PARTIAL + reconciliation note.** PAR-BF-CORE-027: CountTokens subset SAT via bf-openai-4; the 20+ request-type constant enum is VAR/ESC (g0router uses string literals + dedicated handlers; most enumerate ESC surfaces — BIFROST-MAP:392 forbids dead constants).
- **PARTIAL + variant/ESC note.** PAR-BF-CORE-028: `GatewayContext` is RequestID-only by deliberate design (bf-core-1 D2); typed-KV/metadata has no live consumer and is plugin-pipeline + tracing coupled (both ESC); no field added.
- **PARTIAL + cite.** PAR-BF-CORE-001: CountTokens subset now SAT via bf-openai-4; the 14 ESC method groups (Rerank/OCR/Video*/Container*/CachedContent*/Passthrough/Compaction) remain ESC — no funded route (D1).
- **ESC (inherited).** Compaction: no funded route → no interface method (would be dead on 43 providers; Wave-5 NewWithShutdown lesson + BIFROST-MAP:392); ESC-REF-ABSENT for wire shape.
- **No Go file changed.** provider.go/errors.go micro-serial with bf-openai-4 is DISCHARGED (bf-openai-4 merged at HEAD ad0cb82). ESC-REF-ABSENT honored — built nothing to a guessed Bifrost wire format. The large bifrost-core escalation set (plugin pipeline, ProviderQueue, pooling, KVStore/clustering, CEL/adaptive-LB/health-states, OTEL/tracing, vector backends — rows 003-018,021-026,029-033,037-050) recorded in open-questions.md per BIFROST-MAP §8. bf-core-2 owns semantic cache (034-036).

## bf-mcp-1 (closeout) - 2026-06-15
MCP **server-mode foundation** shipped (Go-only; FIRST holder of the routes_admin.go MCP serial chain). g0router is now exposable AS an MCP server.
- **NEW surfaces.** `internal/mcp/server.go` (server-mode JSON-RPC dispatcher: `initialize`/`tools/list`/`tools/call`, reusing the shipped `splitFrames` + `NewBridgeToolExecutor` + `smartFilterText`; no `mark3labs/mcp-go` dep). `internal/server/routes_mcp.go` (`RegisterMCPRoutes`: `POST /mcp` raw JSON-RPC + `GET /mcp` SSE + the session-gated complete-oauth route). Additive `MCPServerPost`/`MCPServerSSE`/`CompleteInstanceAuth`/`resolveMCPVK`/`mcpToolCatalog`/`assembleServerCatalog` on `*admin.Handlers`; `SetMCPServerHeartbeat` additive setter.
- **D3 one catalog.** `/mcp tools/list` and `/api/mcp/tools` (`ListTools`) both map from the SINGLE `mcpToolCatalog()` source — no second source of truth.
- **D4 VK live consumer.** `resolveMCPVK` (`x-g0-vk > Bearer > x-api-key`, header names VAR) is genuinely consumed: a provided-but-unknown/inactive VK is REJECTED via `store.GetVirtualKeyByKey`; absent VK allowed (optional surface; per-VK scoping = bf-mcp-2).
- **D5 hermetic heartbeat.** `: ping\n\n`/15s via an INJECTED ticker + in-memory sink (zero real elapsed time; no socket in any test).
- **D7 dead-method revival.** `CompleteInstanceAuth` is the FIRST live caller of the shipped-but-dead `Engine.Complete`; returns the masked account (no token echoed).
- **D8 deferred SSE finalizer (real payload).** The finalizer runs a best-effort `recordAudit("mcp_server.tools_call", …)` stamping the resolved VK AFTER the sink closes (not during frame emission) — 054 HAVE, not a no-op.
- **D6 zero new columns.** Existing `mcp_clients` + `mcp_oauth_*` (`*_enc`) satisfy the server-mode/encryption subset; no server-mode path consumed a new column → none added. `migrate.go` UNTOUCHED.
- **Matrix flip (`bifrost-mcp.md`).** 002/003/052/053/054 → HAVE; 075 → HAVE-by-variant; 032/039 → PARTIAL. ESC items (per-VK scoping, mandatory-VK, enterprise columns, per-user OAuth/sessions, create-vs-update, mcp-go dep, numeric usage shape, ESC-REF-ABSENT) in `open-questions.md`.
- **Closeout.** ESC-REF-ABSENT honored (built to matrix + base JSON-RPC/SSE spec only — no guessed Bifrost framing). `/mcp` NOT in LOCAL_ONLY_PATHS (D2). routes_openai.go/guard.go/the consume-only `internal/mcp/{bridge,sse,probe,agent,oauth,…}.go` UNTOUCHED. routes_admin.go serial slot RELEASED to bf-mcp-2. `go build/vet/test ./...` green (1996 tests / 43 pkgs). Not committed: `ui/dist/index.html`.

## bf-mcp-2 (closeout) - 2026-06-15

Per-VK MCP server-mode scoping + tool filtering. Makes the SHIPPED `/mcp` surface per-VK scoped: when a resolved+admitted VK has an assignment, its `tools/list` is filtered to its `executeOnlyTools` scope and `tools/call` is gated to the same set; an absent/un-assigned VK keeps the full global catalog (regression-tested).

- **D1 lazy-creation VARIANT.** ONE global server; the per-VK scope is computed on-demand at request time (`admin/mcp.go newMCPServer(vk)` → `scopedServerTools`) — no long-lived per-VK server-object map (that architecture is VAR, recorded).
- **D2 additive junction table.** NEW `virtual_key_mcp_configs` (`migrate.go`) + `internal/store/mcpvkconfigs.go` store (many-to-many VK↔client; mirrors `mcp_tool_groups`); the filter's live SOURCE via `ListVKMCPConfigsByVK`.
- **D3/D4 live narrowing + reuse.** NEW PURE `internal/mcp/scope.go scopeTools` REUSES the shipped `stripServerPrefix` dual-key primitive (no re-derived prefix logic); matches `*` / `<client>-*` / `<client>-<tool>`. Enforced on BOTH `tools/list` (scoped `CatalogSource`) and `tools/call` (`scopedDispatcher` gate). Test proves a restricted VK sees strictly fewer tools + cannot call an out-of-scope tool.
- **D5/049 subset validation.** PURE `validateAutoExecuteSubset` called in the LIVE assignment create/update path; rejects `autoExecute ⊄ execute` with a 4xx `{error}` before storage (test proves the reject).
- **D6 AllowOnAllVirtualKeys bypass.** Per-client `allow_on_all_virtual_keys` flag (`mcp_clients.config_json`); the scoped source UNIONs an all-VK client's tools into every VK (test proves bypass on/off).
- **D7 DisableAutoToolInject (HAVE).** Per-client `disable_auto_tool_inject` flag; a LIVE read in `scopedServerTools` omits a flagged client's tools from the scoped surface (test proves suppression).
- **D8 flags + hash.** `config_hash` SHA-256 computed on write + EXPOSED in the assignment GET DTO for drift-detection (test asserts the DTO carries it + changes on update; reconciliation worker ESC). `allowed_extra_headers` stored + canonicalized on client write (forwarding ESC). `is_code_mode_client` stored (VFS execution ESC, never faked). `ToolAnnotations` additive `omitempty` shape on `ServerTool` carried through `tools/list` (probe supplies no data → shape present, no data).
- **Matrix flip (`bifrost-mcp.md`).** 004 → HAVE-by-variant; 017/019/020/033/049/057 → HAVE; 018/071/077/078/079 → PARTIAL.
- **Serial.** routes_admin.go MCP slot (via `routes_mcp.go RegisterMCPRoutes`) held for the additive `/api/mcp/vk-configs` CRUD lines only; RELEASED to bf-core-2. The bf-mcp-1 `/mcp` wire contract + `resolveMCPVK`/`admitMCPVK` validation UNCHANGED. Consume-only `internal/mcp/{filter,allowlist,toolpolicy,…}.go` UNTOUCHED. No `init()`, no mark3labs/mcp-go dep, no destructive DDL. `go build/vet/test ./...` green (2029 tests / 43 pkgs). Not committed: `ui/dist/index.html`.

## bf-core-2 (closeout) - 2026-06-15

Semantic cache (g0router-shaped), phase-19 EXACT-KEY-HASH half only. Greenfield `internal/semcache/` domain: `sha256(normalized prompt+model)` (`keys.go`) → O(1) SQLite read-through/write-through (`cache.go`). A flag-gated hook in the NON-STREAM chat path SHORT-CIRCUITS the provider on a hit and writes through on a miss; the SEMANTIC-SIMILARITY half (cosine + embedder) is DEFERRED — not built — pending the operator embedding-source decision.

- **D1/D2 exact-key HAVE, semantic half DEFERRED.** `Lookup`/`Store` do exact-key SQLite only — NO embedder, NO cosine engine, NO candidate loader, NO semantic branch (building them now would be production-inert for zero parity gain; recorded open-questions §bf-core-2).
- **D3 additive table.** NEW `semantic_cache` table + 2 indexes (`migrate.go`, phase-19:34-46 verbatim) + `internal/store/semcache.go` exact-key CRUD (get-by-key non-expired, insert, hit++, lazy purge, list, stats, clear). `embedding_json` retained (stores `[]`) for forward-compat — data, not dead code.
- **D4/D6 positional, non-stream only.** The read-through hook sits after the x-g0-vk gate / before `provider.ChatCompletion` (where a guardrail check belongs — guardrails do not run inline today, documented). Stream branch never consults the cache.
- **D5/D8 config + flag.** TTL via `cache_ttl_seconds` setting; `semantic_cache` feature flag seeded OFF (`IsFeatureEnabled` by-key, fail-OFF). Threshold/Provider/EmbeddingModel/Dimension out-of-scope-now with the deferred half.
- **D7 lazy purge.** `expires_at` filter + `PurgeExpiredSemanticCache` on the read path; injected clock (hermetic) — NO background reaper/Ticker/init.
- **Admin.** NEW `internal/admin/cache.go` GET `/api/cache/semantic` (stats + key/model/hits/expires — NO full response_json leaked) + DELETE (audited `semantic_cache.clear`). Wired via additive `chat.SetSemanticCache` (`routes_openai.go` adapter; `NewChatHandler` signature unchanged).
- **Matrix flip (`bifrost-core.md`).** 034/035/036 → PARTIAL; 037/038 → ESC (SQLite+Go-cosine design divergence).
- **Serial.** routes_admin.go serial chain TERMINATES at bf-core-2 (bf-mcp-1 → bf-mcp-2 → bf-core-2) — appended GET+DELETE `/api/cache/semantic`, releases to NOBODY. ESC-REF-ABSENT honored (built to phase-19:60-69, no Bifrost vector shape). Strict TDD red→green. No `init()`, no global state, errors-as-values, additive-only DDL. `go build/vet/test ./...` green (2064 tests / 44 pkgs). Not committed: `ui/dist/index.html`.

## bf-gov-4 (VK mandatory mode) - 2026-06-15

VK mandatory mode shipped (Go-only, no new routes, no UI). `vk_mandatory` feature flag seeded OFF via the bf-core-2 INSERT-OR-IGNORE loop (`migrate.go`) — backward-compatible by construction (absent VK allowed on fresh/upgraded stores). Additive `mandatory func() bool` field + `SetMandatoryChecker` setter on `VKGate` (`internal/api/vk.go`); `AllowVK`'s empty-key branch consults the predicate: OFF/nil → allowed (today's behavior), ON → `(false, 401, "virtual key required", nil)`. The 10 `/v1/*` call-site `if vkHeader != ""` guards removed (Option-A, authorized) so `AllowVK("")` is reachable; per-request predicate wired in `routes_openai.go` via `SetMandatoryChecker(func() bool { ok, _ := st.IsFeatureEnabled("vk_mandatory"); return ok })`. `/mcp` mirror: `h.vkMandatory()` helper + absent-VK mandatory branch in `admitMCPVK` (`internal/admin/mcp.go`); reason threaded through to callers ("virtual key required" distinct from "virtual key unknown or inactive"). Both surfaces enforce the identical flag. Row 034 flipped MISSING → HAVE (variant header: `x-bf-vk → x-g0-vk`, authorized per BIFROST-MAP §250). The AUDIT-FOUND unassigned BUILD gap is closed. ESC-REF-ABSENT honored (built to matrix behavior + g0router conventions only). TDD: 7 commits (3 RED + 4 GREEN); `go build/vet/test ./...` green (43 pkgs). Not pushed (orchestrator verifies).

## bf-gov-5 (VK value encrypted at rest) - 2026-06-16

VK bearer value now encrypted at rest (GOV-006). The `virtual_keys.key` column holds `sha256hex(raw)` for O(1) lookup and a NEW additive `key_enc TEXT NOT NULL DEFAULT ''` column holds the reversible AES-256-GCM ciphertext (`internal/store/crypto.go` cipher + `sha256hex` helper). `CreateVirtualKey` inserts hash+enc (raw never persisted in plaintext); `scanVirtualKey` (promoted to a `*Store` method) decrypts `key_enc` back into `.Key` on every read so DTO display, spend attribution, and the VK gate all keep seeing the raw `g0vk-` value; `GetVirtualKeyByKey` hashes its input before the `WHERE key = ?` lookup. `UpdateVirtualKey` already omits both columns (regression-guarded, no impl change). BLOCKER fixed in the same commit group: `SumCostByTeam` (`internal/store/requestlog.go`) can no longer correlate `request_log.api_key` against the now-hashed `key` column via raw SQL (SQLite has no sha256), so it was rewritten Go-mediated — decrypt the team's `key_enc` values, bind the raw keys into a dynamic `IN (?,…)` list (empty ⇒ return 0 guard) — keeping team-budget enforcement live. Legacy plaintext rows are migrated by an idempotent in-`Open` backfill (`backfillVirtualKeyEncryption`, `key_enc=''` guard) sequenced after Store construction — `Open(path, secret)` signature UNCHANGED; no operator lockout (original raw key still resolves). Additive-only migration (`ensureColumn`), no DROP/rebuild. Row 006 flipped MISSING → HAVE. RESIDUAL (honestly recorded in the matrix Notes): `request_log.api_key` still stores the raw VK value for spend attribution — encrypting it is a separate larger hardening, out of GOV-006 scope (VK record/table only). TDD: RED-before-GREEN per step (1, 2, A, B, C, D, E); the four load-bearing tests (`TestCreateVirtualKeyStoredAtRest`, `TestVirtualKeyRoundTripPlaintext`/`TestGetVirtualKeyByKeyResolvesRaw`, `TestSumCostByTeamSurvivesEncryption`, `TestBackfillNoLockout`, `TestSpendAttributionKeyIsRaw`) all green via the new code paths. `go test ./...` (2091 tests / 44 pkgs) + `go vet ./...` green. Not pushed (orchestrator verifies). `ui/dist/index.html` left untouched.

## bf-mcp-sat (closeout) - 2026-06-15

bifrost-mcp client-mode SAT verification closed as **matrix-flip-only + one additive regression test** (NO production code): audited the 17 MAP-routed client-mode candidates against the LIVE Wave-7 tree; honestly flipped 11 (007/011 → HAVE; 001/005/006/021 → PARTIAL; 024 → HAVE-by-variant; 025/067/076 → VAR; 080 → VAR-narrow) each with a cited file:line, and left 6 MISSING/ESC (008/009/010/026/027/066) with cited absence proofs — the MAP §263 "flip all 17" over-claim corrected, parity NOT inflated. Locked the 067 free-form status lifecycle with `TestMCPInstanceStatusLifecycle`. No `internal/mcp/*.go` production edit (consume-only, BIFROST-MAP §397); not a routes serial holder (MCP chain terminated at bf-core-2). ESC-REF-ABSENT honored. **bifrost-mcp has no further buildable rows** — the residual is the ESC per-user/enterprise surface.

## bf-mcp-3 (MCP instance env encrypted at rest) - 2026-06-16

MCP instance env secrets now encrypted at rest (PAR-BF-MCP-080), mirroring the bf-gov-5 pattern. A NEW additive `mcp_instances.env_json_enc TEXT NOT NULL DEFAULT ''` column (`migrate.go` `ensureColumn` loop) holds the reversible AES-256-GCM ciphertext of the instance env map; `CreateMCPInstance`/`UpdateMCPInstance` encrypt `in.Env` into `env_json_enc` and write `'{}'` into the legacy plaintext `env_json` column so a secret never persists in plaintext (drain-on-write). `GetMCPInstance`/`ListMCPInstances` SELECT `env_json_enc` (the `env_json` projection dropped); `scanMCPInstance` decrypts it back into `.Env` (fail-closed, mirror `mcpoauth.go`), with an empty-string guard treating `env_json_enc == ''` as empty env rather than calling `Decrypt("")`. Legacy plaintext rows are migrated by an idempotent in-`Open` backfill (`backfillMCPInstanceEnvEncryption`, `env_json_enc=''` guard) sequenced after Store construction as a sibling of the VK backfill — `Open(path, secret)` signature UNCHANGED; no operator lockout / launch-survival (decrypted `.Env` still returns the original secrets). Additive-only migration, no DROP/constraint change. Scope: `env_json` only — `url`/`command`/`args_json` are non-secret and stay plaintext (args excluded per plan Q1). Residual honestly recorded: stored env is not currently re-fed to the launcher (the single `StartStdio` call uses request-body env), but the row no longer persists plaintext secrets. Row 080 flipped MISSING (ESC) → HAVE. TDD: 6 commits (RED before each GREEN per steps 1, 2, 3, 4); the load-bearing tests (`TestMCPInstanceEnvEncryptedAtRest`, `TestMCPInstanceEnvRoundTrip`, `TestMCPInstanceEnvUpdateNoPlaintext`, `TestMCPInstanceBackfillNoLockout`, `TestMCPInstanceBackfillIdempotent`) all green via the new code paths. `go test ./...` + `go vet ./...` green. Not pushed (orchestrator verifies). `ui/dist/index.html` left untouched.

## Phase bf-followups — PLANNED (post-encryption review) - 2026-06-16

Forward-looking phase (NOT a closeout — nothing started). Created from a ponytail over-engineering review of the bf-gov-5 + bf-mcp-3 code plus the residuals those closeouts recorded honestly. All items are scoped/additive and governed by `CLI_ORCHESTRATOR.md`; per the WORKFLOW rule, create a wave before implementing any of these. Each carries its own status — do not treat the phase as a single unit.

| Task | Status | What | Decision gate |
|------|--------|------|---------------|
| **bf-refactor-1** | PENDING | Ponytail shrink (`net: -35 lines`): the two near-identical backfills `backfillVirtualKeyEncryption` (`internal/store/virtualkeys.go`) and `backfillMCPInstanceEnvEncryption` (`internal/store/mcpinstances.go`) share the same collect-then-update skeleton (query `WHERE <enc>=''` → collect `pending` slice → `rows.Close` → loop `Encrypt`+`UPDATE`). Extract one `(s *Store) backfillEnc(table, srcCol, encCol string, transform func(raw string) (newSrc, enc string, err error)) error`; VK passes `transform → sha256hex(raw), Encrypt(raw)`, MCP passes `→ "{}" , Encrypt(raw)`. Centralizes the single-conn collect-then-update correctness concern in one place. Pure simplification — no behavior change; existing backfill/no-lockout tests are the regression guard. | None — ready to build. |
| **bf-gov-6** | BLOCKED (NEEDS-DECISION) | Close the bf-gov-5 residual: `request_log.api_key` still stores the **raw** VK value at rest (for spend attribution). Encrypting/hashing it would require moving the attribution correlation (`SumCostByAPIKey`, `SumCostByTeam`, usage views) off the raw key onto a hash or VK-id, touching the quota engine + usage subsystem. | Operator must decide whether to invest; non-trivial cross-cutting change. |
| **bf-mcp-6** | BLOCKED (PRODUCT DECISION) | The *stored* (now-encrypted) MCP instance env is never re-fed to the launcher — the single `StartStdio` call (`internal/admin/mcp.go`) uses the inbound request-body env, so `env_json_enc` is currently consumed only by the session-gated DTO. Decide whether the launcher should read the stored env on (re)start (and if so, wire it through the decrypting `scanMCPInstance`). | Product decision: is stored-env launch the intended behavior? |
| **bf-mcp-7** | PENDING (CONDITIONAL) | Extend at-rest encryption to `mcp_instances.args_json` IF operators are observed passing secrets via CLI args (`--token=…`). Excluded from bf-mcp-3 scope (args are an allowlisted argv, typically non-secret). Mechanically identical to bf-mcp-3 — trivial mirror when justified. | Trigger: evidence of secrets-in-args; otherwise leave as-is. |

Note: these are quality/decision follow-ups, NOT new parity rows — the bifrost buildable parity set remains exhausted (everything else MISSING is the deferred re-architecture or other NEEDS-DECISION items in `open-questions.md`).
