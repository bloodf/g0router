...
| Remediation stages | Stage 7, Waves 7.A–7.M; Stage 8 completion hardening |
| Max parallel agents | 8 |
| Critical path | Stage 0 → 1 → 2 → 3 → 4 → 5 → 6 → remediation → completion hardening |
...
error hardening, quota capability truth hardening, OpenRouter quota support, and
provider quota matrix documentation through Wave 8.CC.
...
STAGE 8 ─── Waves 8.L–8.CC       ──► tracked completion hardening, E2E/API integration, audit fixes, provider parity, search/streaming/quota hardening
```
