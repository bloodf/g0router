...
| `vertex` | `vertex` | `vertex` | `vertex` | `supported` | OAuth/service account | yes | yes | yes | no | yes | yes | Provider-qualified catalog IDs such as `vertex/gemini-2.5-flash` route through the native Vertex adapter when `VERTEX_PROJECT_ID` and `VERTEX_LOCATION` are configured; Vertex auth uses the Gemini OAuth flow but persists runtime provider `vertex`; streaming and quota fetcher are not implemented. |
